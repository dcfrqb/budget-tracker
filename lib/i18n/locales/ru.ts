// ─────────────────────────────────────────────────────────────
// Russian locale — source of truth for all UI strings
// ─────────────────────────────────────────────────────────────

// DictShape<T> — widens all leaf strings to `string` for structural typing.
// Used so enDict can be typed against the same shape without requiring
// identical literal string values.
type DictShape<T> = {
  [K in keyof T]: T[K] extends string
    ? string
    : T[K] extends object
    ? DictShape<T[K]>
    : never;
};

export const ruDict = {
  common: {
    add: "Добавить",
    close: "Закрыть",
    no_data: "нет данных",
    no_deadline: "без срока",
    unit: {
      hour: "ч",
      day: "д",
    },
    month: {
      short: {
        jan: "янв",
        feb: "фев",
        mar: "мар",
        apr: "апр",
        may: "май",
        jun: "июн",
        jul: "июл",
        aug: "авг",
        sep: "сен",
        oct: "окт",
        nov: "ноя",
        dec: "дек",
        "1":  "янв",
        "2":  "фев",
        "3":  "мар",
        "4":  "апр",
        "5":  "май",
        "6":  "июн",
        "7":  "июл",
        "8":  "авг",
        "9":  "сен",
        "10": "окт",
        "11": "ноя",
        "12": "дек",
      },
    },
    period: {
      "7d":  "7д",
      "30d": "30д",
      "90d": "90д",
      "1y":  "1г",
      "1m":  "1м",
      "3m":  "3м",
      "6m":  "6м",
      "12m": "12м",
      ytd:   "YTD",
      this_month:   "этот месяц",
      this_quarter: "этот квартал",
      this_year:    "этот год",
    },
    weekday: {
      short: {
        sun: "вс",
        mon: "пн",
        tue: "вт",
        wed: "ср",
        thu: "чт",
        fri: "пт",
        sat: "сб",
      },
    },
  },

  settings: {
    title: "настройки",
    subtitle: "заглушка",
    soon: "скоро",
    description:
      "Профиль, источники дохода, налоговая ставка, валюта по умолчанию, языки, hotkeys, темы, экспорт данных, управление группой. Соберём после того, как первые 8 вкладок отработают в реальных сценариях.",
    locale: {
      sectionTitle: "Язык интерфейса",
      label: "Язык",
      option: {
        ru: "Русский",
        en: "Английский",
      },
    },
    timezone: {
      label: "Часовой пояс",
      help: "Используется для группировки транзакций и аналитики по дням.",
      option: {
        other: "Другой…",
      },
      invalid: "Неверный часовой пояс",
    },
    autosync: {
      section_title: "Автосинхронизация",
      hint: "Применяется ко всем интеграциям. Для срочного — кнопка «Синхр.» в Транзакциях.",
      option: {
        "2h":  "2 ч",
        "12h": "12 ч",
        "24h": "24 ч",
        off: "Выкл",
      },
    },
    default_period: {
      section_title: "Период по умолчанию",
      hint: "Применяется ко всем разделам, когда период не выбран явно.",
      option_all: "всё",
    },
    profile: {
      section_title: "Профиль",
      field_name: "Имя",
      field_gender: "Пол",
      gender_male: "Мужской",
      gender_female: "Женский",
      gender_unspecified: "Не указан",
      save: "Сохранить",
      saved: "Сохранено",
      placeholder_name: "Ваше имя",
      error_save: "Ошибка сохранения",
    },
    budget: {
      section_title: "Бюджет",
      mode_label: "Режим",
      mode_economy: "Эконом",
      mode_normal: "Норма",
      mode_free: "Свобода",
      currency_label: "Основная валюта",
      saved: "Сохранено",
      error_save: "Ошибка сохранения",
    },
    categories_summary: {
      section_title: "Категории",
      summary: "{active} {activeWord}, {archived} {archivedWord}",
    },
    work_sources_summary: {
      section_title: "Источники дохода",
      summary: "{count} {word}",
      summary_zero: "нет источников",
    },
    accounts_summary: {
      section_title: "Счета и институции",
      summary: "{accounts} {accountsWord}, {institutions} {institutionsWord}",
    },
    integrations: {
      section_title: "Интеграции",
      gated_title: "Только в admin-режиме",
      gated_body: "Подключение интеграций доступно только в admin-режиме. Функция в разработке.",
      page_title: "Интеграции",
      page_body: "Интеграции с внешними сервисами. В разработке (Фаза J).",
      manage: "Открыть",
      title: "Интеграции с банками",
      warning_title: "Внимание: ToS-риски",
      warning_body: "Интеграции с банками имеют риски нарушения условий обслуживания. Используйте только на свой страх и риск. Credentials шифруются AES-256-GCM с ключом из INTEGRATIONS_ENC_KEY.",
      admin_required_title: "Только для администратора",
      admin_required_body: "Интеграции доступны только в admin-режиме. Установите ADMIN_INTEGRATIONS=true в .env для активации.",
      available: "Доступные адаптеры",
      connected: "Подключённые интеграции",
      no_credentials: "Нет подключённых интеграций",
      adapter: {
        tinkoff_csv: "Тинькофф — CSV-выгрузка",
        tinkoff_email: "Тинькофф — Email-уведомления",
        tinkoff_retail: "Т-Банк — личный кабинет",
        generic_csv: "Универсальный CSV",
      },
      status: {
        disconnected: "Отключён",
        connected: "Подключён",
        needs_otp: "Ожидает OTP",
        error: "Ошибка",
      },
      action: {
        connect: "Подключить",
        login: "Войти",
        submit_otp: "Ввести OTP",
        sync: "Синхронизировать",
        disconnect: "Отключить",
        delete: "Удалить",
      },
      form: {
        username: "Телефон / логин",
        password: "Пароль",
        otp_code: "Код OTP",
        display_label: "Метка (опционально)",
        email_forward: "Email для пересылки",
        pin: "PIN-код",
        pin_hint: "4 цифры — выбираешь сам",
        pin_placeholder: "••••",
        phone_placeholder: "+7 999 123-45-67",
        lk_password: "Пароль ЛК Т-Банка",
        lk_password_hint: "Текущий пароль от tbank.ru — нужен для входа после SMS.",
        lk_password_placeholder: "••••••••",
      },
      category: {
        csv: "CSV-файл",
        email: "Email-уведомления",
        api_reverse: "API (reverse)",
      },
      bybit_card: {
        title: "Bybit Card",
        subtitle: "Карта Bybit (Mastercard) — импорт расходов",
        api_key_label: "API ключ",
        api_secret_label: "API секрет",
        show_secret: "Показать",
        hide_secret: "Скрыть",
        test_connect_cta: "Проверить и подключить",
        helper: "Read-only HMAC ключ. Создай в Bybit → API → New Key, привяжи к IP сервера приложения.",
        connecting: "Подключаем…",
        connected: "Подключено",
        error: {
          invalid_key: "Неверный ключ или подпись",
          network: "Ошибка сети",
          rate_limit: "Лимит запросов исчерпан",
          unknown: "Неизвестная ошибка",
        },
        last_sync: "Последняя синхронизация: {when}",
        sync_now: "Синхронизировать",
        disconnect: "Отключить",
        sync: {
          success: "Синхронизировано: {created} новых, {skipped} уже было",
          empty: "Новых транзакций нет",
          error: {
            region_block: "Bybit недоступен с этого IP. Нужен прокси-узел.",
            auth: "Ключ API недействителен",
            rate_limit: "Bybit временно ограничил запросы. Повтори позже.",
          },
        },
        balance: {
          synced: "Баланс обновлён: {amount} {currency}",
          partial: "Баланс частично: {amount} {currency} (пропущено: {sources})",
          failed: "Не удалось получить баланс: {reason}",
        },
      },
      tinkoff_retail: {
        title: "T-Банк (личный)",
        description: "Подключение к личному кабинету T-Банка через reverse-API для автоматической синхронизации транзакций.",
        connect: {
          pin_intro: "PIN-код задаёшь ты сам — это требование Т-Банка для привязки устройства. Мы храним его в зашифрованном виде, чтобы при следующих синхронизациях входить без SMS.",
        },
        relogin: {
          title: "Переподключить",
          password_only_hint: "Сессия истекла. Введите пароль заново — телефон сохранён.",
        },
        link: {
          title: "Привязать счета",
          column: {
            tbank_account: "Счёт в T-Банке",
            currency: "Валюта",
            local_account: "Локальный счёт",
          },
          unlinked_option: "(не связан)",
          save: "Сохранить связи",
          no_external_accounts: "Счетов не найдено. Подтвердите SMS, чтобы получить список.",
          manage_button: "Привязки счетов",
          linked_count: "Связано счетов: {count}",
          loading: "Загружаем счета из T-Банка…",
          create_and_link: "Создать счёт и привязать",
          created: "Создан и привязан",
        },
        error: {
          insufficient_privileges: "Недостаточно прав на этом аккаунте Т-Банка.",
          invalid_credentials: "Неверный телефон или PIN.",
          rate_limited: "Слишком много попыток. Попробуйте позже.",
          circuit_open: "Слишком много ошибок подряд — синхронизация временно остановлена.",
          unknown: "Не удалось связаться с T-Банком.",
          invalid_pin: "PIN должен быть из 4 цифр.",
          invalid_phone: "Неверный формат телефона.",
          captcha_required: "Т-Банк показал капчу. Попробуй позже или войди вручную через приложение.",
          session_expired: "Сессия Т-Банка устарела. Подключи интеграцию заново.",
          sms_timeout: "SMS не пришла или код не введён вовремя.",
          no_session: "Нет сохранённой сессии — подключи интеграцию заново.",
          no_pending_sms: "Сейчас никто не ждёт SMS-код.",
          login_failed: "Не удалось завершить вход.",
          unknown_step: "Неизвестный экран на странице входа. Логи отправлены — нужна диагностика.",
          sms_input_missing: "Не появилось поле для SMS-кода. Попробуйте позже.",
          pin_screen_missing: "Не появился экран PIN-кода. Попробуйте позже.",
          mybank_redirect_missing: "Не удалось дождаться перехода в ЛК. Попробуйте ещё раз.",
          push_confirmation_required: "Подтвердите вход в приложении T-Bank на телефоне.",
          too_many_transitions: "Слишком много экранов в процессе входа. Попробуйте ещё раз позже.",
          invalid_otp: "Неверный код подтверждения.",
          invalid_password: "Неверный пароль ЛК.",
          too_many_attempts: "Превышено количество попыток. Подождите и попробуйте позже.",
          network_error: "Сеть недоступна. Проверь интернет и попробуй позже.",
          navigation_timeout: "Т-Банк отвечает медленно. Попробуй ещё раз через минуту.",
          browser_unavailable: "Внутренний браузер не запустился. Это проблема на нашей стороне.",
          invalid_session: "Сохранённая сессия повреждена. Подключи интеграцию заново.",
          lk_password_required: "Введите пароль от ЛК Т-Банка.",
          tinkoff_session_cookies_missing: "Не удалось извлечь сессионные куки T-Bank. Переподключи интеграцию.",
          create_and_link_failed: "Не удалось создать счёт. Попробуй ещё раз.",
        },
        action: {
          relogin: "Переподключить",
          manage_links: "Импортировать счета",
        },
      },
      sync: {
        result: "+{created} / пропущено {skipped}",
        ok: "ok",
        last_sync: "синк:",
        in_progress: "Синхронизация…",
        completion: {
          title_ok: "Синхронизация завершена",
          title_error: "Синхронизация прошла с ошибкой",
          created: "Создано {count}",
          updated: "Обновлено {count}",
          errors: "Ошибок {count}",
          go_transactions: "Перейти в транзакции",
          close: "Закрыть",
        },
      },
      cadence: {
        label:   "автосинк",
        "1h":    "1 ч",
        "6h":    "6 ч",
        "12h":   "12 ч",
        "24h":   "24 ч",
        off:     "выкл",
        next_in: "след. через {when}",
      },
    },
    export: {
      section_title: "Экспорт данных",
      description: "Скачать все ваши данные в формате JSON.",
      button: "Скачать всё (JSON)",
    },
    danger: {
      section_title: "Опасная зона",
      wipe_button: "Стереть все мои данные",
      dialog_title: "Подтверждение удаления",
      dialog_body: "Это удалит все ваши транзакции, счета, кредиты, подписки, фонды, источники дохода и другие данные. Действие необратимо.",
      confirm_label: "Введите «{phrase}» для подтверждения",
      confirm_phrase: "УДАЛИТЬ",
      confirm_button: "Удалить все данные",
      cancel_button: "Отмена",
      error_wipe: "Ошибка при удалении данных",
    },
    link: {
      manage: "Управлять →",
    },
  },

  summary: {
    settings: {
      mode: "режим",
      modeValue: "норма",
      view: "вид",
      viewValue: "настройки",
    },
    home: {
      mode_key: "режим",
      period_key: "период",
      period_val: "30д",
      mode_economy: "эконом",
      mode_normal: "норма",
      mode_free: "свобода",
      // === wave-1+2 shell ===
      cashflow: {
        delta_pos: "+{amount} к пред. неделе",
        delta_neg: "−{amount} к пред. неделе",
      },
    },
    transactions: {
      filter_label: "фильтр",
      filter_meta: "30д · все",
      found_key: "найдено",
      inflow_key: "приток",
      outflow_key: "отток",
      avg_day_key: "ср / день",
      mode_key: "режим",
      mode_val: "норма",
      view_key: "вид",
      view_val: "лента",
      pending_key: "ожидание",
      pending_val: "{count} плановых",
      pending_none: "нет",
      // === wave-1+2 shell ===
      pending_tooltip: "Плановые транзакции ожидают подтверждения",
    },
    income: {
      month_label: "доход в этом мес",
      fact_key: "факт",
      upcoming_label: "ближ. поступления",
      upcoming_meta: "окно 14д",
      upcoming_none: "нет ближайших поступлений",
      mode_key: "режим",
      mode_val: "норма",
      view_key: "вид",
      view_val: "доходы",
      nearest_key: "ближайшее",
      nearest_none: "нет",
    },
    expenses: {
      reserve_label: "резерв 30д",
      loans_key: "кредиты 30д",
      subs_key: "подписки/мес",
      subs_label: "подписки · тариф/мес",
      personal_key: "только ты",
      split_key: "шеринг",
      paid_for_others_key: "за других",
      total_key: "итого",
      mode_key: "режим",
      mode_val: "норма",
      reserve_key: "резерв",
      subs_count_key: "подписок",
    },
    planning: {
      funds_label: "накопления · итого",
      goal_total_key: "цель всего",
      progress_key: "прогресс",
      contrib_month_key: "взнос / мес",
      events_label: "ближайшие события",
      events_meta: "14д окно",
      events_none: "нет событий в ближ. 14 дней",
      mode_key: "режим",
      mode_val: "норма",
      view_key: "вид",
      view_val: "планир.",
      nearest_key: "ближ. дата",
      nearest_none: "нет событий",
    },
    analytics: {
      period_label: "период · {period}",
      avg_net_label: "в среднем за месяц",
      avg_net_unit: "₽/мес",
      avg_net_sub: "средний нетто",
      weather_label: "погода · статус",
      savings_key: "норма накоплений",
      status_key: "статус",
      delta_label: "топ прирост · категории",
      no_data: "нет данных",
      flow_label: "поток · {period}",
      flow_meta: "live",
      mode_key: "режим",
      mode_val: "норма",
      view_key: "вид",
      view_val: "аналитика",
      weather_key: "погода",
      weather_sun: "солнечно",
      weather_cloud: "облачно",
      weather_rain: "дождь",
      weather_storm: "шторм",
    },
    wallet: {
      net_label: "итоговый баланс",
      accounts_sub: "всего по {n} счетам",
      inst_share_label: "доля по институциям",
      inst_share_meta: "от итого",
      ccy_label: "балансы по валютам",
      no_accounts: "нет активных счетов",
      accounts_label: "счета",
      mode_key: "режим",
      mode_val: "норма",
      view_key: "вид",
      view_val: "кошелёк",
      accounts_key: "счетов",
    },
    family: {
      space_label: "пространство",
      space_meta: "переключить",
      space_shared: "Общее",
      space_personal: "Личное",
      space_hint: "Новая транзакция по умолчанию создаётся в общем. Меняй при добавлении, если нужно.",
      no_group_label: "группа",
      no_group_meta: "не создана",
      no_group_hint: "Создай семейную группу на вкладке «Семья» чтобы видеть общий баланс здесь.",
      mode_key: "режим",
      mode_val: "норма",
      group_key: "группа",
      group_none: "нет",
    },
  },

  expenses: {
    loans: {
      section_title: "Кредиты",
      empty: "пока нет кредитов",
      add: "+ Кредит",
      amortization: "амортизация",
    },
    long_projects: {},
    kpi: {
      loans: "КРЕДИТЫ",
      loans_sub: "/мес · {count} активен",
      subs: "ПОДПИСКИ",
      subs_sub: "/мес · {count} активно",
      utilities: "ЖКХ + ДОМ",
      taxes: "НАЛОГИ · ПОДСКАЗКИ",
      taxes_sub: "платить вручную",
      projects: "ДОЛГИЕ ПРОЕКТЫ",
      projects_unit: "шт",
      projects_sub: "{count_label} · {budget} бюджет",
      section_title: "обзор обязательств",
      section_meta: "текущий месяц",
    },
    category: {
      usage_label: "{pct}% от общих расходов",
      total_of: "из {total}",
      project_category: "категория: {name}",
      section_title: "категории",
      section_meta: "текущий месяц",
      section_sort: "по сумме",
      empty: "нет данных по категориям",
    },
    badge: {
      personal: "Личная",
      split: "Шеринг",
      paid_for_others: "Плачу за всех",
    },
    interval: {
      monthly: "месячно",
      quarterly: "раз в квартал",
      semiannual: "раз в полгода",
      annual: "годовая",
      custom: "каждые {n} мес",
    },
    subscriptions: {
      pageTitle: "подписки",
      manage: "Управлять подписками",
      totals: {
        personal_label: "только ты",
        split_label: "шеринг",
        paid_for_others_label: "за других",
      },
      summary: {
        activeCount: "{count} активно",
        monthly: "{amount}/мес",
        personalLabel: "только ты",
        splitLabel: "шеринг",
        paidForOthersLabel: "за других",
        addButton: "+ Добавить",
      },
      group: {
        personal: {
          title: "личные",
          subtitle: "· {count} ·",
          empty: "нет личных подписок",
        },
        split: {
          title: "шеринг",
          subtitle: "· {count} ·",
          empty: "нет шеринга",
        },
        paidForOthers: {
          title: "за других",
          subtitle: "· {count} ·",
          empty: "ни за кого не плачу",
        },
      },
      card: {
        nextPayment: "след. {date}",
        perMonth: "/мес",
        myShare: "моя доля {amount}",
        sharedWith: "шеринг с {count}",
        markPaid: "Отметить",
        intervalMonthly: "ежемесячно",
        intervalQuarterly: "раз в квартал",
        intervalYearly: "раз в год",
        intervalCustom: "каждые {n} мес",
        variable_badge: "ср. за 3 мес",
      },
      variable: {
        field_label: "Плавающая цена",
        field_help: "цена меняется каждый период",
        price_label_estimate: "Ориентир цены",
        keywords_caption: "Распознаётся по",
        keywords_empty: "нет алиасов",
      },
      history: {
        title: "История платежей",
        empty: "Нет связанных платежей",
        source_auto: "авто",
        source_manual: "вручную",
        source_cash: "наличные",
        unlink: "Отвязать",
        unlink_with_rollback: "Отвязать и откатить дату",
        variance_plus: "+{pct}%",
        variance_minus: "{pct}%",
      },
      suggestions: {
        section_title: "Возможные платежи по подпискам",
        reason_alias_ambiguous: "неоднозначный алиас",
        reason_similarity: "похоже на",
        reason_amount_date: "сумма + дата",
        confirm: "Это {name}",
        dismiss: "Скрыть",
        count_more: "+{n} ещё",
      },
      duplicates: {
        section_title: "Похоже на дубликаты",
        action_merge: "Объединить",
        action_dismiss: "Это разные",
      },
      merge: {
        dialog_title: "Объединить подписки",
        pick_keep_label: "Какую подписку оставить?",
        confirm_button: "Объединить",
        warning_archived: "Вторая подписка будет заархивирована, а все её платежи перенесутся в оставленную.",
        error_generic: "Ошибка — попробуйте ещё раз",
        action_merge: "Объединить",
        action_not_duplicate: "Это разные",
        clear_selection: "Сбросить",
        selected_count: "Выбрано: {count}",
        bar_label: "Выбрано подписок: {count}",
        need_two_hint: "Выберите ровно 2 подписки",
        select_label: "Выбрать",
        deselect_label: "Снять выбор",
      },
      reimbursement: {
        section_title: "Возможные возвраты",
        reason_amount_match: "сумма совпадает",
        reason_amount_and_name_match: "сумма + имя",
        confirm: "Это возврат за {name}",
        dismiss: "Скрыть",
        no_spend_hint: "Нет расхода для привязки",
        income_label: "Поступление",
        spend_label: "Расход",
        sub_label: "Подписка",
        from_label: "от {name}",
      },
      pay: {
        mode_link: "Привязать списание",
        mode_manual: "Записать вручную",
        link_title: "Выберите транзакцию",
        link_empty: "Нет подходящих транзакций",
        link_confirm: "Привязать",
      },
      import: {
        button: "Импортировать",
        title: "Импорт подписок",
        hint: "JSON-массив объектов. Образец ниже.",
        sample_label: "Пример",
        submit: "Загрузить",
        success: "Создано: {n}",
        error_row: "Строка {index}: {message}",
        parse_error: "Невалидный JSON",
      },
      json: {
        button: "JSON",
        title: "Подписки · JSON",
        empty_hint: "Список пуст — ниже пример формата.",
        valid: "Валидно",
        parse_error: "Ошибка JSON: {message}",
        schema_error: "Ошибка схемы: {path} — {message}",
        diff_summary: "Создать: {created} · Обновить: {updated} · Удалить: {deleted}",
        confirm_apply: "Применить?",
        cancel: "Отмена",
        apply: "Применить",
        applying: "Применяю…",
      },
      fromSelection: {
        dialog_title: "Подписка из транзакций",
        mode_create: "Создать новую",
        mode_link: "Привязать к существующей",
        field_name: "Название",
        field_name_placeholder: "Например: Netflix",
        field_variable_price: "Плавающая цена",
        field_interval: "Период списания",
        field_interval_1: "Ежемесячно",
        field_interval_3: "Раз в квартал",
        field_interval_6: "Раз в полгода",
        field_interval_12: "Раз в год",
        field_currency: "Валюта",
        submit_create: "Создать и привязать",
        submit_link: "Привязать",
        select_subscription: "Выберите подписку",
        loading_subscriptions: "Загрузка…",
        no_subscriptions: "Нет активных подписок",
        selected_count: "{count} транз.",
        error_internal: "Ошибка — попробуйте ещё раз",
        error_conflict: "Одна из транзакций уже привязана",
        error_not_found: "Объект не найден",
      },
    },
    filter: {
      label_section: "РАЗДЕЛ",
      label_period: "ПЕРИОД",
      section_all: "Все",
      section_loans: "Кредиты",
      section_subs: "Подписки",
      section_projects: "Проекты",
      section_taxes: "Налоги",
      period_30d: "30д",
      period_90d: "90д",
      period_1y: "1г",
      period_all: "всё",
    },
    credit_cards: {
      title: "Кредитные карты",
      tag: "CARD",
      min_payment: "Минимальный платёж",
      due_in_days: "Через {days}д",
      due_today: "Сегодня",
      overdue: "Просрочен",
      pay_button: "Оплатить",
      debt_label: "Долг",
      empty: "нет кредитных карт с обязательствами",
    },
  },

  income: {
    kpi: {
      ytd_sub: "{months} мес · ср {avg} / мес",
      sources_sub_work: "{work} работа · {freelance} фриланс",
      sources_empty: "нет источников",
      tax_sub: "только подсказки · вручную",
      rate_sub_primary: "из основного источника",
      rate_sub_empty: "нет данных",
      inflow_label: {
        "30d": "За 30 дней",
        "90d": "За 90 дней",
        "1y": "За 365 дней",
        all: "За всё время",
      },
      inflow_sub: "{days} д · ср {avg} / мес",
      sources_label: "АКТИВНЫХ ИСТОЧНИКОВ",
      tax_label: "НАЛОГ НАЧИСЛЕНО",
      rate_label: "ЧАСОВАЯ СТАВКА",
      // === wave-3 polish ===
      empty_cta_title: "Добавь источник дохода",
      empty_cta_body: "Чтобы видеть налоги, ставку в час и расчёт «сколько часов до покупки».",
      empty_cta_button: "+ Добавить источник",
      sources_hint_title: "доход есть, источники не настроены",
      sources_hint_body: "Добавь источник дохода, чтобы видеть налоги, часовую ставку и расчёт времени до покупки.",
      section_title: "доходы",
    },
    work_sources: {
      add: "+ Добавить источник",
      empty: "Нет источников дохода — добавь работу или фриланс",
    },
    sources: {
      comparison: {
        title: "Сравнение источников",
        empty: "Источников меньше двух — сравнивать нечего",
      },
    },
    work: {
      kind_label: {
        employment: "Работа",
        freelance: "Фриланс",
        one_time: "Разовое",
        source: "Источник",
      },
      tax: {
        ndfl: "НДФЛ {pct}%",
        szn: "Самозан. {pct}%",
        none: "Без налога",
      },
      card: {
        last_payment: "Последняя выплата",
        mtd_total: "С начала месяца",
        typical_monthly: "Типичный (среднее за 3 мес)",
        next_expected: "Следующая ожидается",
        no_payments: "Выплат пока нет",
        tax_label: "НАЛОГ",
      },
      detail: {
        back: "Все источники",
        edit: "Редактировать",
        deactivate: "Деактивировать",
        activate: "Активировать",
        period: {
          "1m": "1 мес",
          "3m": "3 мес",
          "6m": "6 мес",
          "12m": "12 мес",
          all: "Всё",
        },
        kpi: {
          total_income: "Доход за период",
          txn_count: "Транзакций",
          avg_per_month: "В среднем за месяц",
          effective_rate: "Эфф. часовая ставка",
          estimated_tax: "Расчётный налог",
          tax_hint: "Σ доход × {pct}%, не учитывает реально уплаченный",
          avg_days_to_pay: "Средн. срок оплаты",
          late_streak: "Подряд просрочены",
          days_unit: "дн.",
        },
        forecast: {
          title: "Прогноз ближайших выплат",
          note: "Прогноз, не записан в базу",
        },
        chart: {
          monthly: "Помесячно",
          cumulative: "Накопительно",
          empty: "Нет данных за выбранный период",
        },
        txns: {
          title: "Транзакции",
          filter: {
            all: "Все",
            done: "Подтверждены",
            planned: "Запланированы",
            partial: "Частично",
          },
          empty: "Привязанных транзакций нет",
          more: "Ещё {count}",
        },
        orders: {
          title: "Заказы",
          empty: "Заказов за период нет",
          paid_prefix: "Оплачено",
          tips_suffix: "чаевые",
          per_hour: "/ч",
          add: "Добавить заказ",
          paid_sum_line: "Получено {paid} / {total} · {count} платежей",
          payments_toggle: "Платежи",
          payments_empty: "Нет привязанных транзакций",
          different_ccy_excluded: "{count} платёж(ей) в другой валюте — не учтены",
          plan_fact: "получено {received} / план {plan} · осталось {remaining}",
          open_full: "Открыть",
          stages_title: "Этапы",
          implicit_stage: "Единый платёж",
          status: {
            planned: "Запланирован",
            active: "В работе",
            awaiting_payment: "Ожидание оплаты",
            completed: "Завершён",
            cancelled: "Отменён",
          },
          grid: {
            view: {
              cards: "Карточки",
              table: "Таблица",
            },
            col: {
              title: "Название",
              client: "Клиент",
              status: "Статус",
              amount: "Сумма",
              currency: "Валюта",
              hours: "Часы",
              rate: "Ставка",
              tips: "Чаевые",
              performed: "Выполнено",
              paid: "Оплачено",
              note: "Заметка",
              received: "Получено",
              remaining: "Остаток",
              actions: "",
            },
            add_row: "+ строка",
            edit_hint: "Нажмите на ячейку для редактирования",
          },
        },
        status_breakdown: {
          title: "Статусы заказов",
          empty: "Нет заказов в периоде",
        },
        status: {
          done: "Подтверждена",
          planned: "Запланирована",
          partial: "Частично",
          missed: "Пропущена",
          cancelled: "Отменена",
        },
        plan: {
          title: "План / факт по месяцам",
          column: {
            month: "Месяц",
            plan: "План",
            fact: "Факт",
            delta: "Δ",
          },
        },
        lifecycle: {
          started: "С {date}",
          ended: "До {date}",
          archived: "Архив",
        },
        currency: {
          base_annotation: "≈ {amount} {ccy}",
        },
        not_found: "Источник не найден",
      },
    },
    expected: {
      status_label: "Ожидаемые",
      empty: "Нет запланированных поступлений",
      events_count: "{count} {word} ближайших 90 дней",
      add: "+ Запланировать поступление",
    },
    freelance_orders: {
      section_title: "Заказы",
      empty: "Заказов пока нет",
      add: "+ Заказ",
      link_txn: "Привязать транзакцию",
      unlink_txn: "Отвязать",
      link_picker: {
        title: "Доступные транзакции",
        empty: "Нет неприсоединённых транзакций по этому источнику",
      },
    },
    order_detail: {
      heading: "Заказ",
      description_label: "Описание",
      payments_history: "История платежей",
      hours_rate: "Часы / Ставка",
      dates: "Даты",
      notes: "Заметки",
      delete: "Удалить заказ",
      delete_confirm: "Удалить этот заказ? Действие необратимо.",
      delete_confirm_title: "Удалить заказ",
      delete_confirm_body: "Удалить «{name}»? Это действие нельзя отменить.",
      back: "Назад к источнику",
      empty_stages: "Этапов нет — заказ оплачивается одним платежом",
      edit: "Редактировать",
      status_label: "СТАТУС",
    },
    today: "сегодня",
    in_days: "+{n}д",
    filter: {
      label_view: "ВИД",
      label_period: "ПЕРИОД",
      view_sources: "Источники",
      view_expected: "Ожидаемые",
      view_other: "Прочее",
      period_30d: "30д",
      period_90d: "90д",
      period_1y: "1г",
      period_all: "всё",
      day_of_month_hint: "день месяца / дней в месяце",
    },
  },

  planning: {
    funds: {
      section_title: "Фонды",
    },
    status_strip: {
      view_label:    "ВИД",
      horizon_label: "ГОРИЗОНТ",
      view_all:      "Всё",
      view_calendar: "Календарь",
      view_funds:    "Накопления",
      view_buys:     "Покупки",
      view_trips:    "Трипы",
      horizon_30d:   "30д",
      horizon_90d:   "90д",
      horizon_1y:    "1г",
      horizon_all:   "всё",
      day_hint:      "день месяца / дней в месяце",
    },
    kpi: {
      saved: "НАКОПЛЕНО",
      saved_sub: "{count} {word}",
      monthly: "ВЗНОС / МЕС",
      monthly_sub: "суммарный взнос",
      next_event: "БЛИЖ. СОБЫТИЕ",
      next_event_none: "нет событий",
      hours: "ЧАСОВ ДО ЦЕЛИ",
      hours_no_rate: "ставка не задана",
      section_title: "план будущего",
      section_subtitle: "обзор",
      funds_count: "{n} {word}",
    },
    fund_kind: {
      TRIP: "трип",
      BUY: "покупка",
      VAULT: "подушка",
      GIFT: "подарок",
      OTHER: "цель",
    },
    event_kind_letter: {
      BIRTHDAY: "Д",
      HOLIDAY: "П",
      TRIP: "Т",
      PURCHASE: "К",
      OTHER: "О",
    },
    purchase_icon: {
      BUY: "К",
      OTHER: "О",
    },
    fund_stat: {
      saved: "накоплено",
      goal: "цель",
      monthly_contrib: "взнос / мес",
      months_left: "осталось мес",
    },
    calendar: {
      section_title: "События",
      events_count: "{n} {word}",
      in_days: " · {n}д",
    },
    upcoming: {
      event_fallback: "событие",
    },
    deadline_none: "без срока",
    hours_rate_label: "ставка {rate}",
    // === wave-3 polish ===
    hours_calc: {
      title: "калькулятор часов",
      subtitle: "сколько работы стоит покупка",
      source_meta: "источник: {name} · {rate} / ч (чистая)",
      price_label: "цена покупки",
      price_hint: "введи сумму и валюту",
      rate_label: "ставка / час (чистая)",
      rate_sub: "{name} · {hpm} ч/мес · {tax}",
      hours_label: "чистые часы работы",
      hours_sub: "≈ {weeks} нед при твоём графике",
      empty_title: "ставка не задана",
      empty_body: "Добавь источник дохода, чтобы видеть стоимость покупки в часах работы.",
      empty_cta: "+ Добавить источник",
    },
    upcoming_dates: {
      title: "ближайшие даты",
      subtitle: "автопредложение отложить",
      meta: "{count} {word}",
      empty: "нет ближайших событий",
    },
    timeline: {
      title: "лента событий",
      subtitle: "горизонт планирования",
      empty_title: "нет событий в периоде",
      empty_body: "Добавь события, подписки, кредиты или накопления, чтобы они появились здесь.",
      today: "сегодня",
      legend: {
        trip: "трипы",
        event: "события",
        subscription: "подписки",
        loan: "кредиты",
        fund_target: "цели",
        txn_planned: "планируемые",
      },
      kind: {
        subscription: "подписка",
        loan_payment: "платёж",
        fund_target: "цель накопления",
        txn_planned: "план. транзакция",
      },
    },
    mini_calendar: {
      title: "календарь",
      subtitle: "два месяца",
      click_hint: "нажми на день",
    },
    calendar_page: {
      h1: "Календарь",
      sub: "визуальная лента событий",
      breadcrumb_planning: "Планирование",
      breadcrumb_calendar: "Календарь",
    },
    trips: {
      list: {
        title: "Трипы",
        subtitle: "поездки и путешествия",
        empty_title: "нет трипов",
        empty_body: "Создай первый трип — добавь бюджет и отслеживай траты.",
        create_cta: "+ Трип",
        archived_section: "Архив",
      },
      card: {
        date_range: "{start} — {end}",
        days_to_start: "через {n} д",
        in_trip: "в поездке",
        past: "завершён",
        status: {
          draft: "черновик",
          planning: "планирование",
          active: "в пути",
          archived: "архив",
        },
        budget_label: "бюджет",
        fund_linked: "фонд привязан",
      },
      create: {
        title: "Новый трип",
        name_label: "название",
        destination_label: "направление",
        start_label: "начало",
        end_label: "конец",
        currency_label: "валюта",
        budget_label: "бюджет",
        fund_section_title: "фонд накоплений",
        fund_none: "без фонда",
        fund_existing: "привязать существующий",
        fund_required_monthly: "взнос / мес (рассчитан)",
        submit: "Создать",
        cancel: "Отмена",
        validation: {
          dates_inverted: "дата начала позже даты окончания",
          budget_positive: "бюджет должен быть > 0",
        },
      },
      detail: {
        breadcrumb: "Трипы",
        countdown_days: "через {n} д",
        countdown_today: "сегодня начало",
        in_trip: "в поездке",
        past: "завершён",
        hero_hours_badge: "{n} ч работы",
        edit: "Изменить",
        delete: "Удалить",
        delete_confirm: "Удалить трип? Это действие нельзя отменить.",
      },
      savings: {
        title: "накопления",
        current: "накоплено",
        goal: "цель",
        required_monthly: "взнос / мес",
        deadline_label: "к старту",
        no_fund_hint: "Привяжи фонд накоплений к этому трипу.",
        link_fund_cta: "Привязать фонд",
        topup_fund_cta: "Пополнить",
      },
      rollup: {
        title: "категории",
        kind: {
          transport: "Транспорт",
          lodging: "Жильё",
          food: "Питание",
          activity: "Активности",
          other: "Прочее",
        },
        col_allocation: "план",
        col_spent: "потрачено",
        col_remaining: "остаток",
        edit_allocation: "изм.",
        save_allocation: "сохр.",
        mixed_ccy_tooltip: "разные валюты, конвертировано в {ccy}",
      },
      bookings: {
        title: "бронирования",
        add_row: "+ Добавить",
        col_kind: "тип",
        col_label: "название",
        col_date: "дата",
        col_amount: "сумма",
        col_status: "статус",
        col_actions: "",
        status: {
          planned: "план",
          paid: "оплачено",
        },
        mark_paid: "Оплатить",
        mark_paid_modal_title: "Списать как расход",
        mark_paid_account_label: "счёт",
        mark_paid_submit: "Подтвердить",
        edit: "Изм.",
        delete: "Удал.",
        delete_confirm: "Удалить бронирование?",
        empty: "нет бронирований",
      },
    },
  },

  analytics: {
    kpi: {
      income_period: "ДОХОД {period}",
      income_sub: "суммарный доход",
      expense_period: "РАСХОД {period}",
      expense_sub: "суммарный расход",
      net_period: "НЕТТО {period}",
      net_sub: "чистый поток",
      net_sub_pos: "положительный",
      net_sub_neg: "отрицательный",
      savings_rate: "норма {pct}%",
      no_data: "нет данных",
      avg_spend_day: "СР. РАСХОД / ДЕНЬ",
      avg_spend_day_sub: "в среднем",
      // === wave-1+2 shell ===
      title: "ключевые метрики",
      title_period: "· {period}",
      period_default: "текущий период",
    },
    forecast: {
      income: "ОЖИД. ДОХОД",
      expense: "ОЖИД. РАСХОД",
      net: "ОЖИД. НЕТТО",
      sub_plan: "план + факт месяца",
      net_pos: "положительный",
      net_neg: "отрицательный",
      title: "прогноз",
      subtitle: "упрощённый · конец периода",
      meta: "без пессимистичного сценария · см. видение",
      empty: "нет данных для прогноза",
      year_net: "ОЖИД. НЕТТО / ГОД",
      year_sub: "экстраполяция среднего",
      year_low_confidence: "мало истории",
      reserved: "ЗАРЕЗЕРВИРОВАНО",
      free: "СВОБОДНО",
      liquid: "ЛИКВИДНО",
      posture_sub: "на текущий момент",
    },
    prescriptive: {
      title: "что можно сделать",
      subtitle: "скорость трат · где сократить",
      meta: "диагностика → рекомендации",
      burn: {
        title: "скорость трат",
        per_day_30: "₽/ДЕНЬ · 30Д",
        per_day_90: "₽/ДЕНЬ · 90Д",
        days_to_zero: "ДНЕЙ ДО НУЛЯ",
        days_to_zero_sub: "при текущей скорости",
        already_negative: "свободных средств нет",
        no_burn: "трат нет",
      },
      shrink: {
        title: "где сократить",
        subtitle: "над своей нормой за 6 мес",
        col_current: "текущий",
        col_avg: "норма",
        col_over: "перерасход",
        empty: "все категории в пределах нормы",
      },
      split: {
        title: "обязательное vs дискреционное",
        subtitle: "на чём можно резать",
        obligatory: "обязательное",
        discretionary: "дискреционное",
        cuttable_pct: "можно резать",
        empty: "нет расходов за период",
      },
      economy_exit: {
        title: "выход из дефицита",
        subtitle: "режим Эконом · сколько месяцев",
        months: "≈ {n} мес в режиме Эконом",
        no_deficit: "уже в плюсе",
        no_surplus: "режим Эконом не создаёт запас",
        deficit_label: "дефицит",
        recovery_label: "запас в месяц",
      },
    },
    mode: {
      ECONOMY: "Эконом",
      NORMAL: "Норма",
      FREE: "Свобода",
      economy_tag: "жёсткие лимиты · экономия",
      normal_tag: "дефолт · всё в порядке",
      free_tag: "мягкие лимиты",
      limit_key: "общий лимит",
      limit_lean: "урезан",
      limit_normal: "стандарт",
      limit_free: "расширен",
      reference: {
        title: "режимы · справка",
        subtitle: "только просмотр · переключать на главной",
        active_on: "активен: {name}",
        active_off: "не задан",
        pill_on: "активен",
        pill_off: "выкл",
        safe_until_label: "безопасно до",
        empty: "режимы не настроены",
      },
    },
    runway: {
      title: "до какого числа прожить",
      until: "до {date}",
      days_remaining: "{n} дней", // TODO: добавить полный plural
      empty: {
        no_limits: "лимиты не заданы для этого режима",
        cta_set_limits: "задать лимиты",
      },
      breakdown: {
        top_categories: "топ категорий",
      },
    },
    trends: {
      title: "тренд",
      subtitle: "доход / расход / нетто",
      empty: "Нет данных за период",
      legend: {
        income:  "доход",
        expense: "расход",
        net:     "нетто",
      },
      y_axis_label: "тыс.",
      curve: {
        approx: "аппроксимация",
        interp: "интерполяция",
        title:  "тип сглаживания графика",
      },
    },
    status_strip: {
      period_label:  "ПЕРИОД",
      compare_label: "СРАВНИТЬ С",
      cmp: {
        prev: "пред. периодом",
        yoy:  "год назад",
        none: "нет",
      },
      day_hint: "день месяца / дней в месяце",
    },
    period: {
      calendar: {
        month:     "месяц",
        quarter:   "квартал",
        year:      "год",
        pick:      "период",
        title:     "выбор периода",
        prev_year: "предыдущий год",
        next_year: "следующий год",
      },
    },
    compare: {
      label_prev: "к предыдущему периоду",
      label_yoy: "год к году",
      label_none: "без сравнения",
      range_caption: "{currentRange} vs {previousRange}",
      title: "сравнение с прошлым периодом",
      disabled: "сравнение отключено",
      no_data_short: "нет данных",
      empty: "нет данных для сравнения",
      summary: {
        rising: "{count} {word} растут",
        falling: "{count} {word} падают",
      },
      col: {
        category: "Категория",
        previous_default: "прошлый",
        current_default: "текущий",
        trend_6m: "тренд 6м",
      },
      delta_new: "новое",
      delta_gone: "пропало",
    },
    weather: {
      title: "финансовая погода",
      subtitle: "здоровье финансов",
      meta: "пересчёт раз в час · ведущий индикатор",
      status_label: "статус",
      scale_label: "шкала состояния",
      classification_label: "классификация",
      kind: {
        sun: "Солнечно",
        cloud: "Облачно",
        rain: "Дождь",
        storm: "Шторм",
      },
      reserve: "запас прочности — {n}/10",
      alert_level: "уровень тревоги — {score}/10",
      matches: "соответствует «{label}»",
      formula: "Погода считается из нормы накоплений и баланса доходов/расходов за выбранный период.",
      reason_prefix: "Причина оценки: {hint}.",
      reason: {
        outflow_gt_inflow_period: "расход превышает доход за выбранный период",
        savings_rate_lt_5pct: "норма накоплений ниже 5%",
        savings_rate_5_to_20pct: "норма накоплений 5–20%",
        savings_rate_gt_20pct: "норма накоплений выше 20%",
      },
      savings_rate: {
        line: "норма накоплений {value}% за выбранный период",
        no_data: "недостаточно данных за выбранный период",
      },
      classification: {
        sun: "полный запас",
        cloud: "всё норм, мелкие сигналы",
        rain: "один из факторов проседает",
        storm: "кризис",
        sun_threshold: "норма > 20%",
        cloud_threshold: "5–20%",
        rain_threshold: "< 5%",
        storm_threshold: "расход > доход за период",
      },
    },
    category_pie: {
      title: "категории расходов",
      period_default: "текущий месяц",
      meta: "{count} {word} · отсорт. по сумме",
      total_label: "ИТОГО",
      legend_period: "период {period}",
      legend_period_default: "текущий период",
      empty: "нет данных по категориям",
    },
  },

  family: {
    members: {
      section_title: "Участники",
    },
    role: {
      owner: "владелец",
      member: "участник",
    },
    fund: {
      no_deadline: "без срока",
      contrib_total: "итого",
      progress_label: "{pct}% · из ₽ {total}",
    },
    sub: {
      sharing: "Шеринг",
      paid_for_others: "Плачу за всех",
    },
    member: {
      balance_key: "сальдо",
    },
    space: {
      personal_tag: "ЛИЧНОЕ",
      personal_name: "моё пространство",
      personal_sub: "виден только мне",
      personal_balance: "личный баланс",
      shared_tag: "ОБЩЕЕ",
      shared_name: "пространство «{name}»",
      shared_name_default: "общее пространство",
      shared_sub: "видно всем участникам",
      shared_balance: "общий баланс",
    },
    status_strip: {
      space_label: "ПРОСТРАНСТВО",
      space_shared: "Общее",
      space_personal: "Моё личное",
      space_all: "Всё вместе",
      period_label: "ПЕРИОД",
      period_month: "этот мес",
      period_90d: "90д",
      period_ytd: "YTD",
      period_all: "всё",
      day_hint: "день месяца / дней в месяце",
    },
    your_share: "твоя доля",
    spaces: {
      title: "пространства",
      subtitle: "личное / общее",
      hint: "переключай при добавлении транзакции",
      empty: "нет пространств",
    },
    balances: {
      title: "сверка",
      subtitle: "кто кому должен",
      settle_btn: "Провести выплату",
      empty: "нет текущих задолженностей",
      algo_hint: "алгоритм: минимальное число переводов чтобы обнулить все разницы",
    },
    shared_funds: {
      title: "общие накопления",
      subtitle: "кто сколько вложил",
      meta: "{count} {word}",
      empty: "нет общих накоплений",
      progress_key: "прогресс",
    },
  },

  transactions: {
    debt_meta: "{count} активно · net",
    toolbar: {
      import_csv: "CSV Импорт",
      import_csv_title: "Импорт транзакций из CSV",
      // === wave-3 polish ===
      search_placeholder: "поиск по описанию",
    },
    sync: {
      button: "Синхронизировать",
      last: "обн {ago}",
      never: "ни разу",
      in_progress: "синхронизация…",
      error: "ошибка",
      no_credentials: "Подключите банк в настройках",
      toast_no_credentials: "Нет подключённых интеграций",
      toast_adapter_ok: "{name}: {count} новых",
      toast_adapter_synced: "{name}: синхронизирован",
      toast_adapter_err: "{name}: ошибка",
      toast_already_running: "Уже идёт",
      toast: {
        launched: "Синхронизация запущена",
        timeout: "Синхронизация зависла",
      },
      button_no_integrations: "нет интеграций",
      button_syncing: "синхр…",
      button_relative: "синхр. {when}",
    },
    feed: {
      title: "лента",
      meta: "новые сверху",
      shown_of: "{shown} из {total}",
      load_more: "Ещё",
      daily_net: "нетто за день",
    },
    new: {
      no_accounts: "Нет счетов. Создайте счёт в разделе Кошелёк.",
    },
    filter: {
      label_type: "ТИП",
      label_period: "ПЕРИОД",
      type_all: "Все",
      type_inc: "Доходы",
      type_exp: "Расходы",
      type_xfr: "Переводы",
      type_loan: "Займы",
      period_7d: "7д",
      period_30d: "30д",
      period_90d: "90д",
      period_1y: "1г",
    },
    period: {
      title: "сводка периода",
      range: "· 30д",
      txn_short: "транз.",
      avg: "ср",
      net_forecast: "прогноз {amount}",
      net_deficit: "дефицит {amount}",
      net_zero: "нетто 0",
      planned_count: "{n} план",
      partial_count: "{n} частично",
      column: {
        inflow: "ПРИТОК",
        outflow: "ОТТОК",
        xfr: "ПЕРЕВОДЫ",
        net: "НЕТТО",
      },
    },
    status: {
      planned: "Запланир.",
      partial: "Частично",
      done: "Выполнено",
      missed: "Пропущ.",
      cancel: "Отменено",
    },
    weekday: {
      sun: "вс",
      mon: "пн",
      tue: "вт",
      wed: "ср",
      thu: "чт",
      fri: "пт",
      sat: "сб",
    },
    day: {
      today: "· сегодня",
      inflow: "приток",
      outflow: "отток",
      planned: "план",
      expected: "{n} ожидается",
      cancelled: "{n} отменено",
    },
    selection: {
      count: "выбрано: {count}",
      aria_select_row: "Выбрать транзакцию",
      mark_as_transfer: "Перевод",
      break_transfer: "Разорвать",
      mark_as_compensation: "Компенсация",
      clear: "Снять выделение",
      tooltip: {
        need_two: "Выберите ровно 2 строки на разных счетах",
        same_account: "Обе строки на одном счёте",
        already_transfer: "Эти строки уже часть перевода",
        already_grouped: "Одна из транзакций уже в группе",
        need_mixed_kinds: "Выберите ≥1 расход и ≥1 доход",
        bad_status: "Можно склеивать только подтверждённые транзакции",
        scope_mismatch: "Все транзакции должны быть в одном пространстве",
        break_link: "Разорвать связь (перевод или компенсацию)",
        break_mixed: "Выделение содержит и переводы, и компенсации — разрывайте по одному типу",
      },
      error: {
        same_kind: "Нельзя пометить как перевод две строки одного направления",
        rate_missing: "Курс недоступен — перевод создан со ставкой 1, отредактируй вручную",
        transfer_not_found: "Перевод не найден или уже удалён",
        compensation_failed: "Не удалось склеить",
      },
      success: {
        marked: "Помечено как перевод",
        broken: "Перевод разорван",
        compensation_made: "Склеено в компенсацию",
        compensation_broken: "Компенсация разорвана",
        subscription_created: "Подписка создана и привязана",
        subscription_linked: "Привязано к подписке",
      },
      subscription: {
        action: "Подписка",
        tooltip_need_expense: "Выберите хотя бы 1 расход без подписки",
        tooltip_already_linked: "Одна из транзакций уже привязана к подписке",
      },
    },
    compensation: {
      error: {
        too_few: "Нужны минимум 2 транзакции",
        same_kind_only: "Нужны и расход, и доход",
        bad_status: "Только подтверждённые транзакции",
        scope_mismatch: "Разные пространства",
        fx_rate_missing: "Нет курса для конвертации",
        edit_blocked: "Разорвите компенсацию для редактирования",
        invalid_group: "недействительная группа",
      },
      badge: {
        label: "комп · {sign}{n}",
      },
      tooltip: "компенсация (+{n})",
      dialog: {
        title: "Группа компенсации",
        netto_label: "нетто",
        members_count: "участников: {n}",
        break_button: "Разорвать компенсацию",
        transfer_label: "Перевод",
      },
    },
    merge: {
      action_label: "Объединить",
      group_noun: "Объединение",
      badge: "объед · {n}",
      tooltip: {
        need_same_sign: "выделите транзакции одного знака",
      },
      error: {
        need_same_sign: "Объединять можно только расходы или только доходы",
        failed: "Не удалось объединить",
      },
      success: {
        merged: "Объединено",
        broken: "Объединение разорвано",
      },
      dialog: {
        title: "Объединение",
        sum_label: "сумма",
        members_count: "участников: {n}",
        break_button: "Разъединить",
      },
    },
  },

  wallet: {
    // === wave-2 parser+wallet+breadcrumbs ===
    totals: {
      section_title:     "итого по кошельку",
      section_sub:       "все счета в ₽",
      rates_updated:     "курсы · обн {ago}",
      rates_never:       "курсы · нет данных",
      net_label:         "чистая сумма",
      net_sub:           "всего по {n} счетам",
      banks_crypto_label: "банки и крипто",
      banks_crypto_sub:  "карты и крипто · без сбережений и наличных",
      savings_label:     "подушка / вклады",
      savings_sub:       "{n} накопительных",
      cash_label:        "наличка",
      cash_sub:          "{n} {word}",
    },
    strip: {
      group: "ГРУППА",
      currency: "ВАЛЮТА",
      group_all: "Все",
      group_banks: "Банки",
      group_crypto: "Крипто",
      group_cash: "Наличка",
      group_arch: "Архив",
      currency_all: "Все",
      day_progress: "д{day}/{days}",
      day_hint: "день месяца / дней в месяце",
    },
    cash_meta: "{locations} {locWord} · {currencies} {curWord}",
    add_account: {
      title: "Добавить новый счёт",
      sub: "ручной ввод или интеграция",
      btn_manual: "Ручной ввод",
      btn_integration: "Интеграция",
    },
    kind: {
      card: "Дебет",
      credit: "Кредит",
      savings: "Накоп.",
      cash: "Наличные",
      crypto: "Биржа",
      loan: "Автосписание",
    },
    collateral: {
      card: "Карта",
      credit: "Кредитная",
      savings: "Накоп.",
      cash: "Наличные",
      crypto: "Крипто",
      loan: "Сервисн.",
    },
    archived: {
      closed: "закрыт",
      recent: "закрыт недавно",
      months: "закрыт {n} мес назад",
      years: "закрыт {n} {word} назад",
    },
    account: {
      refreshed: "обн",
      refreshed_ago: "обн {ago}",
      badge: {
        not_in_analytics: "не в аналитике",
      },
      edit: {
        archive: "Архивировать",
        archive_failed: "Не удалось архивировать счёт",
        requisites_section: "Банковские реквизиты",
      },
      credit: {
        debt: "Долг",
        debt_inline: "Долг: {amount}",
        available: "Доступно",
        limit: "Лимит",
        no_limit: "Лимит не задан",
      },
      form: {
        include_in_analytics: {
          label: "Учитывать в аналитике",
          hint: "Влияет на safe-until, доступную сумму и агрегаты",
        },
        credit: {
          rate: "Ставка годовых (%)",
          limit: "Кредитный лимит",
          grace_period: "Беспроцентный период (дней)",
          statement_day: "День выписки (1–31)",
          min_payment: {
            label: "Минимальный платёж",
            percent: "Процент от долга (%)",
            fixed: "Фиксированная сумма",
            type_percent: "Процент",
            type_fixed: "Фиксированная",
          },
        },
        savings: {
          rate: "Ставка годовых (%)",
          capitalization: "Капитализация",
          withdrawal_limit: "Лимит снятия (опционально)",
          cap_none: "Нет капитализации",
          cap_monthly: "Ежемесячная",
          cap_quarterly: "Ежеквартальная",
          cap_yearly: "Ежегодная",
        },
      },
      requisites: {
        title: "Реквизиты",
        account_number: "Номер счёта",
        bic: "БИК",
        bank_name: "Банк",
        inn: "ИНН",
        kpp: "КПП",
        corr_account: "Корр. счёт",
        copy: "Копировать",
        copied: "Скопировано",
        pull_from_tbank: "Подтянуть из T-Банка",
        pulling: "Загрузка...",
        pull_failed: "Не удалось подтянуть реквизиты",
        payment_due_day: "День платежа",
        recipient_name: "Получатель",
      },
    },
    cash: {
      section_title: "наличка",
      section_sub: "· разные места и валюты",
      add_location: {
        title: "Новая локация наличных",
        placeholder_location: "Название локации (напр. «сейф дома»)",
        placeholder_balance: "Начальный остаток",
        submit: "Добавить",
        cancel: "Отмена",
      },
      edit: {
        title: "Редактировать локацию",
        archive: "Архивировать",
        archive_failed: "Не удалось архивировать счёт",
        save: "Сохранить",
        cancel: "Отмена",
      },
    },
    fx: {
      add_pair: {
        button: "+ добавить курс",
        title: "Добавить пару валют",
        from_label: "Из",
        to_label: "В",
        submit: "Добавить",
        cancel: "Отмена",
        unsupported: "Валюта не поддерживается ЦБ РФ",
        same_currency_error: "Выберите разные валюты",
        duplicate_error: "Эта пара уже добавлена",
      },
      remove_pair: {
        title: "Убрать пару",
      },
      section_title: "курсы обмена",
      section_sub: "официальные · для переоценки",
      refresh: "Обновить курсы",
      unavailable: "—",
      fetched_ago: "обновлено {n} мин. назад",
      source: {
        cbr: "ЦБ РФ",
        db: "из кэша",
      },
      updated_label: "обн {ago}",
      stale_warning: "возможно устарели",
    },
  },

  forms: {
    common: {
      save: "Сохранить",
      cancel: "Отмена",
      required: "Обязательное поле",
      required_hint: "* — обязательные поля",
      invalid_amount: "Некорректная сумма",
      invalid_date: "Некорректная дата",
      loading: "Загрузка...",
      delete: "Удалить",
      archive: "Архивировать",
      restore: "Восстановить",
      add: "Добавить",
      edit: "Редактировать",
      form_error: {
        internal: "Внутренняя ошибка. Попробуйте ещё раз.",
        unique_violation: "Запись с такими данными уже существует.",
        not_found: "Запись не найдена.",
        conflict: "Конфликт: операция не разрешена.",
      },
      kind: {
        income: "Доход",
        expense: "Расход",
        transfer: "Перевод",
        loan_payment: "Платёж по кредиту",
        debt_out: "Выдал в долг",
        debt_in: "Получил / вернул",
      },
      scope: {
        personal: "Личное",
        shared: "Общее",
      },
      status: {
        planned: "Запланировано",
        partial: "Частично выполнено",
        done: "Выполнено",
        missed: "Пропущено",
        cancelled: "Отменено",
      },
    },
    tx: {
      title_create: "Новая транзакция",
      title_edit: "Редактировать транзакцию",
      title_income: "Новый доход",
      title_expense: "Новый расход",
      field: {
        kind: "Тип",
        account: "Счёт",
        fromAccount: "Со счёта",
        toAccount: "На счёт",
        category: "Категория",
        amount: "Сумма",
        currency: "Валюта",
        occurred_at: "Дата",
        planned_at: "Плановая дата",
        status: "Статус",
        name: "Название",
        note: "Примечание",
        scope: "Область",
        loan: "Кредит",
        subscription: "Подписка",
        fund: "Фонд",
        long_project: "Проект",
        work_source: "Источник дохода",
        work_source_required: "Источник дохода *",
        planned_event: "Событие",
        personal_debt: "Долг",
      },
      placeholder: {
        name: "Название транзакции",
        note: "Заметка (опционально)",
        category: "— выберите —",
        account: "— выберите счёт —",
        currency: "— валюта —",
      },
      errors: {
        transfer_same_account: "Счёт-источник и счёт-получатель должны быть разными",
        transfer_currency_mismatch: "Переводы между счетами с разными валютами пока не поддерживаются",
        work_source_required: "Выберите источник дохода",
      },
      work_source: {
        empty_cta_title: "Сначала добавьте источник дохода",
        empty_cta_body: "Каждое поступление должно быть привязано к источнику. Создайте хотя бы один.",
        empty_cta_button: "Добавить источник",
      },
    },
    transfer: {
      title_create: "Новый перевод",
      title_edit: "Редактировать перевод",
      field: {
        from_account: "Со счёта",
        to_account: "На счёт",
        from_amount: "Сумма списания",
        to_amount: "Сумма зачисления",
        rate: "Курс",
        fee: "Комиссия",
        occurred_at: "Дата",
        note: "Примечание",
      },
      placeholder: {
        from_account: "— откуда —",
        to_account: "— куда —",
        note: "Примечание (опционально)",
        rate: "авто",
      },
      same_account_error: "Счёт-источник и счёт-получатель должны быть разными",
      cross_currency_hint: "Разные валюты — укажите курс",
    },
    account: {
      title_create: "Новый счёт",
      title_edit: "Редактировать счёт",
      credit: {
        balance_hint: "Введите остаток на карте (сколько ещё можно потратить). Долг рассчитается автоматически: лимит − остаток.",
      },
      errors: {
        credit_rate_required: "Ставка обязательна для кредитных счетов",
        credit_limit_required: "Кредитный лимит обязателен",
        savings_rate_required: "Ставка обязательна для сберегательных счетов",
        cash_goes_through_cash_stash: "Наличка добавляется через раздел наличных, не через форму счёта",
        balance_required: "Укажите начальный баланс",
        card_last4_already_bound: "Эта карта уже привязана к другому счёту",
      },
      field: {
        institution: "Банк / Организация",
        name: "Название счёта",
        kind: "Тип счёта",
        currency: "Валюта",
        balance: "Начальный баланс",
        balance_required: "Стартовый баланс *",
        balance_label_credit: "Остаток на карте (доступно)",
        note: "Примечание",
      },
      kind: {
        card: "Карта",
        credit: "Кредитная карта",
        savings: "Сберегательный",
        cash: "Наличные",
        crypto: "Крипто",
        loan: "Кредитный",
      },
      placeholder: {
        institution: "— выберите или создайте —",
        name: "Название счёта",
        note: "Примечание (опционально)",
      },
      new_institution: "+ Новая организация",
      institution_section: "Новая организация",
      institution_name: "Название",
      institution_kind: "Тип",
      institution_kind_bank: "Банк",
      institution_kind_crypto: "Крипто",
      institution_kind_cash: "Наличные",
      cardLast4: {
        label: "Привязанные карты",
        hint: "Последние 4 цифры карт. Используется для импорта CSV.",
        placeholder: "Введите 4 цифры и нажмите Enter",
        chip_remove_aria: "Удалить карту {last4}",
        error: {
          format: "Должно быть ровно 4 цифры",
          duplicate: "Эта карта уже добавлена",
        },
      },
      section: {
        requisites: {
          title: "Реквизиты",
        },
      },
      requisites: {
        accountNumber: {
          label: "Номер счёта",
          hint: "20 цифр (для РФ)",
        },
        bic: {
          label: "БИК",
          hint: "9 цифр банка",
        },
        bankName: {
          label: "Название банка",
        },
      },
      balance: {
        label: {
          edit: "Текущий остаток",
        },
        hint: {
          edit: "Изменение перепишет остаток вручную и обновит дату последнего изменения",
        },
      },
    },
    category: {
      title: "Категории",
      back: "Назад в настройки",
      field: {
        name: "Название",
        kind: "Тип",
        parent: "Родительская",
        icon: "Иконка",
        color: "Цвет",
        limit_economy: "Лимит Эконом",
        limit_normal: "Лимит Норма",
        limit_free: "Лимит Свобода",
        limit_economy_pct: "Лимит Эконом, % от среднего за 6 мес",
        limit_normal_pct: "Лимит Норма, % от среднего за 6 мес",
        limit_free_pct: "Лимит Свобода, % от среднего за 6 мес",
      },
      kind: {
        income: "Доход",
        expense: "Расход",
      },
      filter_all: "Все",
      filter_income: "Доходы",
      filter_expense: "Расходы",
      filter_archived: "Архив",
      add_button: "+ Добавить категорию",
      empty: "Нет категорий",
      archived_badge: "архив",
      essential_label: "обязательная",
      essential_hint: "не режется при экономии",
      limit_pct_hint: "Пусто = глобальный множитель режима ({pct}%)",
      placeholder: {
        name: "Название категории",
        parent: "— без родителя —",
        icon: "emoji или код",
        color: "hex или имя",
      },
    },
    freelance_order: {
      title_create: "Новый заказ",
      title_edit: "Редактировать заказ",
      delete: "Удалить заказ",
      delete_confirm: "Удалить этот заказ?",
      delete_failed: "Не удалось удалить",
      field: {
        title: "Название заказа",
        description: "Описание",
        client: "Клиент",
        amount: "Сумма",
        hours: "Часы",
        hourly_rate: "Часовая ставка",
        tips: "Чаевые",
        status: "Статус",
        performed_at: "Выполнено",
        paid_at: "Оплачено",
        note: "Заметка",
        rate_mode: "Тип ставки",
      },
      rate_mode: {
        hourly: "Почасовая",
        fixed: "Фикс за проект",
      },
      placeholder: {
        title: "Название заказа",
        description: "Описание заказа (опционально)",
        client: "Имя клиента",
        note: "Заметка (опционально)",
      },
      status: {
        planned: "Запланирован",
        active: "В работе",
        awaiting_payment: "Ждёт оплату",
        completed: "Оплачен",
        cancelled: "Отменён",
      },
      hint: {
        hours_info: "Часы — справочно, не влияют на сумму",
        rate_info: "Часовая ставка — справочно",
      },
    },
    freelance_order_stage: {
      field: {
        label: "Название этапа",
        expected: "Ожидаемая сумма",
        due_date: "Дата исполнения",
      },
      status: {
        pending: "Ожидает",
        paid: "Оплачен",
      },
      mark_paid: "Оплатить",
      unmark: "Отменить оплату",
      attach_txn: "Привязать транзакцию",
      add_stage: "+ Этап",
      delete_stage: "Удалить этап",
      delete_stage_confirm: "Удалить этот этап?",
      sum_mismatch_warning: "Сумма этапов не совпадает с суммой заказа",
      pay_account_label: "Счёт",
      pay_amount_label: "Сумма оплаты",
    },
    work: {
      title_create: "Новый источник дохода",
      title_edit: "Редактировать источник дохода",
      delete: "Удалить источник",
      delete_confirm: "Удалить этот источник дохода?",
      delete_failed: "Не удалось удалить",
      currency_locked: "Валюту нельзя сменить — есть привязанные транзакции",
      field: {
        kind: "Тип",
        name: "Название / Работодатель",
        currency: "Валюта",
        pay_day: "День выплаты",
        tax_rate: "Налоговая ставка (%)",
        hours_per_month: "Часов в месяц",
        is_active: "Активен",
        notes: "Заметки",
        rate_type: "Тип ставки",
        rate_amount: "Сумма ставки",
        rate_hourly: "Ставка / час",
        rate_monthly: "Чистая зарплата / мес",
        rate_per_task: "Сумма за проект",
        rate_daily: "Дневная ставка",
        rate_commission: "Комиссия (%)",
        premium_amount: "Премии (средн./мес)",
        premium_note: "Заметка о бонусах",
        started_at: "Дата начала",
        ended_at: "Дата окончания",
      },
      kind: {
        not_set: "— не указан",
        employment: "Трудоустройство",
        freelance: "Фриланс",
        one_time: "Разовый",
      },
      section: {
        basics: "Основное",
        pay_details: "Реквизиты выплат (справочно)",
        period: "Период",
      },
      rate_type: {
        hourly: "Почасовая",
        monthly: "Месячная",
        per_task: "За проект",
        daily: "Дневная",
        commission_pct: "Комиссия %",
      },
      errors: {
        required_for_employment: "Обязательно для постоянной работы",
        required_for_monthly: "Нужно при месячной ставке",
        commission_out_of_range: "Комиссия должна быть 0–100%",
        ended_before_started: "Дата окончания раньше начала",
        required_when_rate_set: "Укажите тип ставки",
      },
      placeholder: {
        name: "Работодатель или название",
        pay_day: "1–31",
        tax_rate: "6, 13...",
        hours_per_month: "160",
        notes: "Заметки (опционально)",
      },
      hint: {
        hours_per_month: "По умолчанию 160 ч/мес",
        tax_rate: "Только подсказка — налоги ведёте вручную",
        currency_default: "Если не выбрана — подставится основная валюта",
        pay_details_reference: "Справочные поля — не влияют на реальный доход, рассчитываемый из транзакций",
      },
    },
    tx_confirm: {
      title: "Подтвердить транзакцию",
      amount_label: "Сумма",
      date_label: "Дата",
      note_label: "Примечание",
      partial: "Частично",
      fully: "Полностью",
      submit: "Подтвердить",
    },
    tx_row: {
      confirm: "Подтвердить",
      edit: "Редактировать",
      delete: "Удалить",
      miss: "Пропустить",
      cancel: "Отменить",
      are_you_sure: "Вы уверены?",
      yes_miss: "Да, пропустить",
      yes_cancel: "Да, отменить",
      // === wave-3 polish ===
      delete_confirm_body: "Транзакция будет удалена. Действие необратимо.",
      yes_delete: "Да, удалить",
    },
    sub: {
      title_create: "Новая подписка",
      title_edit: "Редактировать подписку",
      field: {
        name: "Название",
        icon: "Иконка",
        icon_color_fg: "Цвет иконки",
        icon_color_bg: "Фон иконки",
        price: "Цена",
        currency: "Валюта",
        interval: "Интервал",
        next_payment: "Следующий платёж",
        sharing: "Тип шеринга",
        total_users: "Кол-во участников",
        family: "Семья",
        note: "Примечание",
      },
      interval_options: {
        m1: "Ежемесячно",
        m3: "Раз в квартал",
        m6: "Раз в полгода",
        m12: "Раз в год",
      },
      sharing_types: {
        personal: "Личная",
        split: "Шеринг (делим)",
        paid_for_others: "Плачу за всех",
      },
      field_is_variable: "Плавающая цена",
      field_is_variable_help: "цена меняется каждый период",
      field_auto_match: "Авторазбор",
      field_auto_match_help: "автоматически привязывать транзакции",
      pay_button: "Оплатить",
      pay_confirm_title: "Подтвердить оплату",
      pay_confirm_body: "Оплатить {name} — {amount}? Следующий платёж сдвинется на {n} мес.",
      delete_confirm_title: "Удалить подписку",
      delete_confirm_body: "Удалить «{name}»? Это действие нельзя отменить.",
      shares_editor: {
        title: "Участники шеринга",
        add: "+ Добавить",
        field_person: "Имя",
        field_amount: "Сумма",
        delete: "Удалить",
        empty: "Участников нет",
      },
      reimbursement: {
        section_title: "Возврат",
        field_expected: "Ожидаемый возврат",
        field_currency: "Валюта возврата",
        field_from: "От кого",
        field_from_placeholder: "Например: Виктория Г.",
      },
    },
    loan: {
      title_create: "Новый кредит",
      title_edit: "Редактировать кредит",
      field: {
        name: "Название",
        principal: "Сумма кредита",
        annual_rate: "Ставка годовых (%)",
        term_months: "Срок (мес)",
        start_date: "Дата начала",
        currency: "Валюта",
        account: "Счёт для платежей",
        note: "Примечание",
      },
      delete_confirm_title: "Удалить кредит",
      delete_confirm_body: "Удалить «{name}»? Это действие нельзя отменить.",
      payment_dialog: {
        title: "Внести платёж",
        field_paid_at: "Дата платежа",
        field_total: "Сумма платежа",
        field_principal: "Тело кредита",
        field_interest: "Проценты",
        field_account: "Счёт списания",
        field_note: "Примечание",
        auto_split: "Автоматически из графика",
        auto_split_hint: "Подставить разбивку из амортизационного графика",
        submit: "Записать платёж",
      },
    },
    fund: {
      title_create: "Новый фонд",
      title_edit: "Редактировать фонд",
      field: {
        kind: "Тип фонда",
        name: "Название",
        currency: "Валюта",
        goal_amount: "Цель (сумма)",
        current_amount: "Уже накоплено",
        target_date: "Дата цели",
        monthly_contribution: "Взнос в месяц",
        note: "Примечание",
      },
      kind_options: {
        trip: "Поездка",
        buy: "Покупка",
        vault: "Подушка безопасности",
        gift: "Подарок",
        other: "Другое",
      },
      delete_confirm_title: "Удалить фонд",
      delete_confirm_body: "Удалить «{name}»? Это действие нельзя отменить.",
      contribute_dialog: {
        title: "Пополнить фонд",
        field_amount: "Сумма",
        field_account: "Счёт списания",
        field_date: "Дата",
        field_note: "Примечание",
        submit: "Пополнить",
      },
    },
    event: {
      title_create: "Новое событие",
      title_edit: "Редактировать событие",
      field: {
        kind: "Тип",
        title: "Название",
        event_date: "Дата",
        repeats_yearly: "Повторяется ежегодно",
        fund: "Связанный фонд",
        expected_amount: "Ожидаемая сумма",
        currency: "Валюта",
        note: "Примечание",
      },
      kind_options: {
        birthday: "День рождения",
        holiday: "Праздник",
        trip: "Поездка",
        purchase: "Покупка",
        other: "Другое",
      },
      delete_confirm_title: "Удалить событие",
      delete_confirm_body: "Удалить «{name}»? Это действие нельзя отменить.",
    },
    long_project: {
      title_create: "Новый проект",
      title_edit: "Редактировать проект",
      field: {
        name: "Название",
        budget: "Бюджет",
        currency: "Валюта",
        category: "Категория",
        start_date: "Дата начала",
        end_date: "Дата окончания",
        note: "Примечание",
      },
      placeholder: {
        name: "Название проекта",
        note: "Примечание (опционально)",
        category: "— без категории —",
        end_date: "без срока",
      },
      delete_confirm_title: "Удалить проект",
      delete_confirm_body: "Удалить «{name}»? Это действие нельзя отменить.",
      has_transactions_hint: "Проект с транзакциями нельзя удалить",
    },
    personal_debt: {
      title_create: "Новый долг",
      title_edit: "Редактировать долг",
      field: {
        direction: "Направление",
        counterparty: "Контрагент",
        principal: "Сумма",
        currency: "Валюта",
        opened_at: "Дата выдачи",
        due_at: "Срок возврата",
        account: "Счёт",
        note: "Примечание",
      },
      direction: {
        OUT: "Я дал в долг",
        IN: "Мне дали в долг",
      },
      placeholder: {
        counterparty: "Имя / Название",
        note: "Примечание (опционально)",
        due_at: "без срока",
        account: "— выберите счёт —",
      },
      delete_confirm_title: "Удалить долг",
      delete_confirm_body: "Удалить долг с «{name}»? Это действие нельзя отменить.",
      has_transactions_hint: "Долг с транзакциями нельзя удалить",
      close_confirm_title: "Закрыть долг",
      close_confirm_body: "Отметить долг с «{name}» как закрытый?",
      payment_dialog: {
        title: "Записать возврат",
        field_amount: "Сумма",
        field_account: "Счёт",
        field_occurred_at: "Дата",
        field_note: "Примечание",
        submit: "Записать",
      },
    },
    family: {
      title_create: "Создать семью",
      title_edit: "Редактировать семью",
      field: {
        name: "Название",
        note: "Примечание",
      },
      placeholder: {
        name: "Название семьи",
        note: "Примечание (опционально)",
      },
      delete_confirm_title: "Удалить семью",
      delete_confirm_body: "Удалить «{name}»? Все данные группы будут удалены.",
    },
    family_member: {
      title_create: "Добавить участника",
      title_edit: "Редактировать участника",
      field: {
        display_name: "Имя",
        letter: "Буква (аватар)",
        color: "Цвет",
        role: "Роль",
      },
      role: {
        OWNER: "Владелец",
        MEMBER: "Участник",
      },
      placeholder: {
        display_name: "Имя участника",
        letter: "авто",
        color: "— выберите —",
      },
      remove_confirm_title: "Удалить участника",
      remove_confirm_body: "Удалить «{name}» из группы?",
      owner_remove_hint: "Владельца нельзя удалить",
      guest_hint: "Участник добавлен как гость — без учётной записи в системе",
    },
  },

  buttons: {
    add_subscription: "+ Подписка",
    add_loan: "+ Кредит",
    add_fund: "+ Фонд",
    add_event: "+ Событие",
    add_project: "+ Проект",
    add_debt: "+ Долг",
    edit: "Редактировать",
    delete: "Удалить",
    pay: "Оплатить",
    contribute: "Пополнить",
    add_payment: "+ Платёж",
    shares: "Участники ({count})",
    add_member: "+ Добавить участника",
    record_return: "Записать возврат",
    close_debt: "Закрыть",
    reopen_debt: "Переоткрыть",
    confirm_delete: "Подтвердить удаление",
    remove_member: "Удалить участника",
  },

  drawer: {
    close: "Закрыть",
    placeholder_title: "Форма появится здесь",
    placeholder_body: "В разработке",
  },

  signals: {
    section_title: "сигналы",
    subtitle: "подсказки, не действия",
    active_count: "{n} активно",
    empty: "нет активных сигналов",
    empty_hint: "появятся при отклонениях в тратах и приближении списаний",
    dismiss: "скрыть",
    item: {
      free_negative: {
        title: "свободных средств нет",
        body: "тратишь из резерва — обязательства не покрыты свободным балансом",
      },
      safe_until_critical: {
        title: "деньги кончатся через {n} дн.",
        body: "при текущем темпе трат запас подходит к нулю — срочно сократи расходы",
      },
      month_deficit: {
        title: "дефицит месяца: {amt}",
        body: "расходы превышают доходы — отрицательный поток за текущий месяц",
      },
      safe_until_low: {
        title: "запас: {n} дн.",
        body: "свободных средств хватит менее чем на три недели",
      },
      obligation_due_soon: {
        title: "{label} через {n} дн.",
        body: "платёж {amt} — убедись, что деньги на счёте",
      },
      obligation_load_high: {
        title: "обязательства {pct}% от свободных",
        body: "ближайшие списания составляют {amt} — больше половины свободного остатка",
      },
      category_overspend: {
        title: "{cat} +{pct}% к прошлому месяцу",
        body: "потрачено {amt} — заметный рост расходов в этой категории",
      },
      expense_over_plan: {
        title: "расходы {pct}% от плана",
        body: "факт {fact} уже превысил план {plan}",
      },
      income_under_plan: {
        title: "доход {pct}% от плана",
        body: "факт {fact} сильно ниже плана {plan}",
      },
      shrinkable: {
        title: "{cat}: перерасход {pct}%",
        body: "превышение среднего на {amt} — категория кандидат на сокращение",
      },
      discretionary_high: {
        title: "необязательные траты {pct}%",
        body: "больше половины расходов — это гибкие статьи: есть что оптимизировать",
      },
      economy_exit_slow: {
        title: "выход из экономии: {n} мес.",
        body: "в режиме Эконом потребуется несколько месяцев, чтобы восстановить баланс",
      },
      mode_free_risky: {
        title: "режим «Свобода» при нестабильных финансах",
        body: "текущий статус не стабильный — рассмотри переключение в Нормальный или Эконом",
      },
      no_outflow_plan: {
        title: "план расходов не задан",
        body: "факт {amt} фиксируется, но сравнить не с чем — добавь плановый бюджет",
      },
      no_inflow_plan: {
        title: "план доходов не задан",
        body: "без планового дохода сложно оценить отклонения",
      },
      category_new: {
        title: "новая категория: {cat}",
        body: "в этом месяце впервые потрачено {amt} — новая статья расходов",
      },
      multi_currency: {
        title: "мультивалютный кошелёк: {n} валюты",
        body: "отслеживай курсы — изменения влияют на общий баланс в базовой валюте",
      },
      burn_rate: {
        title: "расход в день: {amt}",
        body: "средний дневной расход исходя из текущего запаса и горизонта",
      },
      safe_until_good: {
        title: "финансовый запас достаточен",
        body: "свободных средств хватит более чем на 90 дней при текущем темпе",
      },
      month_surplus: {
        title: "профицит месяца: {amt}",
        body: "доходы превышают расходы — можно направить остаток на цели",
      },
      obligations_clear: {
        title: "нет списаний ближайшие 30 дней",
        body: "ни одного обязательного платежа в горизонте месяца",
      },
    },
  },

  home: {
    top_categories: {
      title:       "топ-категорий",
      delta_label: "vs прош. месяц",
      meta:        "топ {count} · по убыв.",
      empty:       "нет данных о расходах по категориям",
      // === wave-3 polish ===
      delta_new: "новое",
    },
    plan_fact: {
      title: "план-факт",
      meta:  "текущий месяц",
      code_inc: "ДОХОД",
      code_exp: "РАСХОД",
      code_net: "НЕТТО",
      // === wave-3 polish ===
      no_plan: "нет плана",
      no_plan_set: "план не задан",
      actual_without_plan: "факт без плана",
      of_amount: "из {amount}",
      net_eom_sub: "кон. мес ≈ {amount}",
      period_label: {
        "7d":  "посл. 7д",
        "30d": "посл. 30д",
        "90d": "посл. 90д",
        "1y":  "посл. год",
      },
      period_meta: {
        "7d":  "последние 7 дней",
        "30d": "текущий месяц",
        "90d": "последние 90 дней",
        "1y":  "последний год",
      },
    },
    status_strip: {
      mode_label:   "РЕЖИМ",
      period_label: "ПЕРИОД",
      modes: {
        econom: "Эконом",
        normal: "Норма",
        free:   "Свобода",
      },
      day_hint:   "день месяца / дней в месяце",
    },
    quick: {
      title: "быстрые действия",
      hotkey_hint: "хоткеи активны · Q",
      income: {
        label: "Доход",
        aria: "добавить доход",
      },
      expense: {
        label: "Расход",
        aria: "добавить расход",
      },
      transfer: {
        label: "Перевод",
        aria: "создать перевод",
      },
      transaction: {
        label: "Транзакция",
        aria: "добавить транзакцию",
      },
      shortcut_hint: "I / E / T для быстрых действий",
    },
    obligations: {
      title: "обязательства",
      period_30d: "ближ. 30д",
      count_suffix: "шт.",
      empty: {
        text: "нет обязательств в ближайшие 30 дней",
        cta: "+ обязательство",
      },
    },
  },

  quick_input: {
    title: "быстрый ввод",
    placeholder: "1500 кофе вчера / зарплата 80000 сегодня",
    confirm: "Записать",
    edit: "Редактировать",
    open_full: "Открыть полную форму",
    hotkey_hint: "Q",
    preview: {
      kind: "тип",
      amount: "сумма",
      category: "категория",
      date: "дата",
      account: "счёт",
    },
    kind: {
      income: "доход",
      expense: "расход",
    },
    error: {
      no_amount: "Не удалось распознать сумму",
      no_account: "Выберите счёт",
      invalid: "Некорректный ввод",
    },
    warning: {
      no_category: "Категория не определена",
      multiple_amounts: "Найдено несколько сумм — взята первая",
      currency_mismatch: "Валюта отличается от счёта",
      category_kind_mismatch: "Категория не соответствует типу транзакции",
    },
    no_accounts: "Нет активных счетов. Создайте счёт в разделе Кошелёк.",
  },

  dialog: {
    confirm_yes: "Да",
    confirm_no: "Отмена",
  },

  onboarding: {
    title: "добро пожаловать",
    subtitle: "настроим бюджет за пару шагов",
    name: {
      label: "Как вас зовут?",
      placeholder: "Ваше имя",
    },
    gender: {
      label: "Пол",
      male: "Мужской",
      female: "Женский",
      unspecified: "Не указывать",
    },
    currency: {
      label: "Основная валюта",
      hint: "Используется для расчётов и отображения балансов",
    },
    mode: {
      label: "Бюджет-режим",
      economy: "Эконом",
      normal: "Нормальный",
      free: "Свободный",
      hint_economy: "Жёсткие лимиты, режим экономии",
      hint_normal: "Стандартные лимиты для большинства",
      hint_free: "Мягкие лимиты, без ограничений",
    },
    submit: "Начать",
    error_save: "Ошибка сохранения. Попробуйте ещё раз.",
  },

  shell: {
    brand: "БДЖ://",
    status: {
      stable: "СТАБИЛЬНО",
      warning: "ВНИМАНИЕ",
      crisis: "КРИЗИС",
    },
    crumbs: {
      home: "обзор",
      txn:  "лента",
      inc:  "источники",
      exp:  "обязательства",
      plan: "календарь",
      anl:  "погода",
      wal:  "счета",
      fam:  "группа",
      // === wave-2 parser+wallet+breadcrumbs ===
      settings: "профиль и бюджет",
      settings_sub: {
        categories:   "категории",
      },
      wal_sub: {
        integrations: "интеграции",
      },
    },
    nav: {
      tabs: {
        home:     "Главная",
        txn:      "Транзакции",
        inc:      "Доходы",
        exp:      "Расходы",
        plan:     "Планирование",
        anl:      "Аналитика",
        wal:      "Кошелёк",
        fam:      "Семья",
        settings: "Настройки",
      },
      codes: {
        home:     "ГЛВ",
        txn:      "ТРН",
        inc:      "ДХД",
        exp:      "РСХ",
        plan:     "ПЛН",
        anl:      "АНЛ",
        wal:      "КШЛ",
        fam:      "СЕМ",
        settings: "НАС",
      },
      aria: "Основная навигация",
    },
    resize: {
      aria_rail:    "Ширина навигации",
      aria_summary: "Ширина сводки",
      title:        "Тащи чтобы растянуть · двойной клик — сброс",
    },
    clock: {
      updated: "обн",
      // === wave-3 polish ===
      tooltip: "Локальное время",
    },
    summary: {
      aria: "Сводка",
      safe: {
        label:        "безопасно ещё",
        label_unsafe: "ЗАПАС ИСЧЕРПАН",
        zero_income:  "@ нулевой доход",
        days:         "дней",
      },
      avail: {
        label:    "доступно сейчас",
        total:    "всего",
        reserved: "резерв",
      },
      balances: {
        label:    "балансы",
        empty:    "нет данных",
        accounts: "{count} счёта",
      },
      session: {
        label:  "состояние сессии",
        online: "онлайн",
        status: "статус",
        integrations: "интеграции",
      },
      quick: {
        label:       "быстро",
        income:      "+ Доход",
        expense:     "+ Расход",
        txn:         "+ Транз",
        tip_income:  "Доход (I)",
        tip_expense: "Расход (E)",
        tip_txn:     "Транзакция (T)",
      },
      cashflow: {
        title:          "денежный поток 30д",
        period_label:   "текущий период",
        empty:          "нет данных",
      },
    },
    sync: {
      busy:    "Синхронизация уже идёт",
      error:   "Ошибка синхронизации",
      empty:   "Нет интеграций",
      tooltip: "Синхронизация",
    },
  },

  import: {
    title: "импорт CSV",
    step_upload: "загрузка",
    step_preview: "предпросмотр",
    step_result: "результат",
    source: {
      label: "Источник",
      tinkoff: "Тинькофф",
      generic: "Другой банк (CSV)",
    },
    encoding: {
      label: "Кодировка файла",
      utf8: "UTF-8",
      win1251: "Windows-1251",
    },
    delimiter: {
      label: "Разделитель",
      semicolon: "Точка с запятой (;)",
      comma: "Запятая (,)",
    },
    file: {
      label: "Загрузить файл .csv",
    },
    csv_paste: {
      label: "Или вставьте CSV-текст",
      placeholder: "Вставьте содержимое CSV-файла сюда...",
    },
    mapping: {
      title: "Маппинг колонок",
      date: "Дата",
      amount: "Сумма",
      currency: "Валюта",
      category: "Категория",
      description: "Описание",
    },
    files: {
      add: "Добавить файлы",
      drop_hint: "или перетащите сюда CSV",
      remove: "Убрать файл",
      target_account: "Счёт назначения",
      source: "Формат",
      row_count: "{n} строк",
    },
    cardhint: {
      title: "Незнакомые карты",
      body: "Карта *{last4} не привязана. Привязать к «{account}»?",
      bind: "Привязать",
      bound: "Привязано",
    },
    preview: {
      included: "Включить",
      duplicate: "Дубликаты",
      total: "Всего строк",
      will_import: "Будет импортировано",
      duplicate_hint: "Вероятно, уже существует в базе",
      transfer_no_category: "—",
      truncated_hint: "Показано 500 из {n} строк. Все строки будут обработаны при импорте.",
      intra_skipped_group: "Внутрикарточные перемещения (пропущено): {n}",
      paired_badge: "↔ парный",
      unpaired_badge: "⚠ без пары",
      unpaired_hint: "Перевод между своими счетами без второй стороны — будет импортирован как обычный доход/расход",
      source_file: "Источник",
    },
    confirm: {
      button: "Импортировать {n}",
      button_with_transfers: "Импортировать {n} (переводов: {t})",
    },
    back: "Назад",
    next: "Далее",
    result: {
      title: "Импорт завершён",
      created: "Создано",
      transfers_created: "Создано переводов: {n}",
      skipped: "Пропущено",
      errors: "Ошибок",
      go_transactions: "В транзакции",
      error_row: "строка {index}: {message}",
    },
    warning: {
      more: "ещё предупреждений",
      bad_row: "некорректная строка",
      unknown_currency: "неизвестная валюта",
      mapping_required: "маппинг не задан",
      parse_error_row: "Ошибка парсинга строки {row}: {message}",
      bad_row_detail: "Строка {row}: некорректные данные ({reason})",
      skipped: "Пропущена строка {row}: {reason}",
      with_file: "{file}: {body}",
    },
    error: {
      no_csv: "Вставьте или загрузите CSV-файл",
      no_account: "Выберите счёт",
      mapping_incomplete: "Укажите колонки для даты и суммы",
    },
  },
  tooltip: {
    abbrev: {
      obn: "обновлено",
    },
  },
  debts: {
    dir: {
      out: "выдал",
      in: "взял",
    },
    next_payment: "следующий {amount} на {date}",
    next_payment_no_date: "следующий {amount} на —",
    returns_count: "{n} возврат{suffix}",
    returns_suffix: {
      one: "",
      few: "а",
      many: "ов",
    },
    no_returns: "без возвратов",
    progress_label: "возвращено {returned} / {total}",
  },
} as const;

export type RuDict = typeof ruDict;
/** Structural shape of ruDict with all leaf strings widened to string. */
export type RuDictShape = DictShape<typeof ruDict>;

/**
 * Plural forms for Russian nouns used with pluralRu() helper.
 * Stored outside ruDict because the i18n Dict type only allows string leaves.
 * Format: [one, few, many] — e.g. ['счёт', 'счёта', 'счетов']
 */
export const ruPluralForms = {
  locations:     ["локация",     "локации",     "локаций"]     as [string, string, string],
  currencies:    ["валюта",      "валюты",      "валют"]       as [string, string, string],
  accounts:      ["счёт",        "счёта",       "счетов"]      as [string, string, string],
  organizations: ["организация", "организации", "организаций"] as [string, string, string],
  transactions:  ["транзакция",  "транзакции",  "транзакций"]  as [string, string, string],
  funds:         ["фонд",        "фонда",       "фондов"]      as [string, string, string],
  events:        ["событие",     "события",     "событий"]     as [string, string, string],
  subscriptions: ["подписка",    "подписки",    "подписок"]    as [string, string, string],
  categories:    ["категория",   "категории",   "категорий"]   as [string, string, string],
  institutions:  ["организация", "организации", "организаций"] as [string, string, string],
  sources:       ["источник",    "источника",   "источников"]  as [string, string, string],
  projects:      ["проект",      "проекта",     "проектов"]    as [string, string, string],
} as const;
