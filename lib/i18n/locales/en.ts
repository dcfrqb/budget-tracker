// ─────────────────────────────────────────────────────────────
// English locale — must mirror ru.ts structure exactly
// TypeScript will error if keys diverge.
// ─────────────────────────────────────────────────────────────

import type { RuDictShape } from "./ru";

export const enDict: RuDictShape = {
  common: {
    add: "Add",
    close: "Close",
    no_data: "no data",
    no_deadline: "no deadline",
    unit: {
      hour: "h",
      day: "d",
    },
    month: {
      short: {
        jan: "Jan",
        feb: "Feb",
        mar: "Mar",
        apr: "Apr",
        may: "May",
        jun: "Jun",
        jul: "Jul",
        aug: "Aug",
        sep: "Sep",
        oct: "Oct",
        nov: "Nov",
        dec: "Dec",
        "1":  "Jan",
        "2":  "Feb",
        "3":  "Mar",
        "4":  "Apr",
        "5":  "May",
        "6":  "Jun",
        "7":  "Jul",
        "8":  "Aug",
        "9":  "Sep",
        "10": "Oct",
        "11": "Nov",
        "12": "Dec",
      },
    },
    period: {
      "7d":  "7d",
      "30d": "30d",
      "90d": "90d",
      "1y":  "1y",
      "1m":  "1m",
      "3m":  "3m",
      "6m":  "6m",
      "12m": "12m",
      ytd:   "YTD",
      this_month:   "this month",
      this_quarter: "this quarter",
      this_year:    "this year",
    },
    weekday: {
      short: {
        sun: "Su",
        mon: "Mo",
        tue: "Tu",
        wed: "We",
        thu: "Th",
        fri: "Fr",
        sat: "Sa",
      },
    },
  },

  settings: {
    title: "settings",
    subtitle: "stub",
    soon: "coming soon",
    description:
      "Profile, income sources, tax rate, default currency, languages, hotkeys, themes, data export, group management. Will be built after the first 8 tabs are tested in real scenarios.",
    locale: {
      sectionTitle: "Interface language",
      label: "Language",
      option: {
        ru: "Russian",
        en: "English",
      },
    },
    timezone: {
      label: "Time zone",
      help: "Used to group transactions and analytics by day.",
      option: {
        other: "Other…",
      },
      invalid: "Invalid time zone",
    },
    autosync: {
      section_title: "Autosync",
      hint: "Applies to all integrations. For immediate sync, use the Sync button on Transactions.",
      option: {
        "2h":  "2h",
        "12h": "12 h",
        "24h": "24 h",
        off: "Off",
      },
    },
    default_period: {
      section_title: "Default period",
      hint: "Applied to all sections when no period is explicitly selected.",
      option_all: "all",
    },
    profile: {
      section_title: "Profile",
      field_name: "Name",
      field_gender: "Gender",
      gender_male: "Male",
      gender_female: "Female",
      gender_unspecified: "Not specified",
      save: "Save",
      saved: "Saved",
      placeholder_name: "Your name",
      error_save: "Save error",
    },
    budget: {
      section_title: "Budget",
      mode_label: "Mode",
      mode_economy: "Economy",
      mode_normal: "Normal",
      mode_free: "Free",
      currency_label: "Primary currency",
      saved: "Saved",
      error_save: "Save error",
    },
    categories_summary: {
      section_title: "Categories",
      summary: "{active} active {activeWord}, {archived} archived {archivedWord}",
    },
    work_sources_summary: {
      section_title: "Income sources",
      summary: "{count} {word}",
      summary_zero: "no sources",
    },
    accounts_summary: {
      section_title: "Accounts & institutions",
      summary: "{accounts} {accountsWord}, {institutions} {institutionsWord}",
    },
    integrations: {
      section_title: "Integrations",
      gated_title: "Admin mode only",
      gated_body: "Connecting integrations is available in admin mode only. Feature is in development.",
      page_title: "Integrations",
      page_body: "Integrations with external services. In development (Phase J).",
      manage: "Open",
      title: "Bank integrations",
      warning_title: "Warning: ToS risks",
      warning_body: "Bank integrations carry Terms of Service violation risks. Use at your own risk. Credentials are encrypted with AES-256-GCM using the INTEGRATIONS_ENC_KEY.",
      admin_required_title: "Admin only",
      admin_required_body: "Integrations are only available in admin mode. Set ADMIN_INTEGRATIONS=true in .env to activate.",
      available: "Available adapters",
      connected: "Connected integrations",
      no_credentials: "No connected integrations",
      adapter: {
        tinkoff_csv: "Tinkoff — CSV export",
        tinkoff_email: "Tinkoff — Email notifications",
        tinkoff_retail: "T-Bank — retail",
        generic_csv: "Generic CSV",
      },
      status: {
        disconnected: "Disconnected",
        connected: "Connected",
        needs_otp: "Awaiting OTP",
        error: "Error",
      },
      action: {
        connect: "Connect",
        login: "Log in",
        submit_otp: "Submit OTP",
        sync: "Sync",
        disconnect: "Disconnect",
        delete: "Delete",
      },
      form: {
        username: "Phone / login",
        password: "Password",
        otp_code: "OTP code",
        display_label: "Label (optional)",
        email_forward: "Forwarding email",
        pin: "Login PIN",
        pin_hint: "4 digits — your choice",
        pin_placeholder: "••••",
        phone_placeholder: "+7 999 123 45 67",
        lk_password: "T-Bank online banking password",
        lk_password_hint: "Current tbank.ru password — required after SMS during connect.",
        lk_password_placeholder: "••••••••",
      },
      category: {
        csv: "CSV file",
        email: "Email notifications",
        api_reverse: "API (reverse)",
      },
      bybit_card: {
        title: "Bybit Card",
        subtitle: "Bybit Card (Mastercard) — expense import",
        api_key_label: "API key",
        api_secret_label: "API secret",
        show_secret: "Show",
        hide_secret: "Hide",
        test_connect_cta: "Test & connect",
        helper: "Read-only HMAC key. Create in Bybit → API → New Key, bind to your app server IP.",
        connecting: "Connecting…",
        connected: "Connected",
        error: {
          invalid_key: "Invalid key or signature",
          network: "Network error",
          rate_limit: "Rate limit hit",
          unknown: "Unknown error",
        },
        last_sync: "Last sync: {when}",
        sync_now: "Sync now",
        disconnect: "Disconnect",
        sync: {
          success: "Synced: {created} new, {skipped} existing",
          empty: "No new transactions",
          error: {
            region_block: "Bybit is unreachable from this IP. Proxy node required.",
            auth: "API key invalid",
            rate_limit: "Bybit rate-limited the request. Try again later.",
          },
        },
        balance: {
          synced: "Balance synced: {amount} {currency}",
          partial: "Balance partial: {amount} {currency} (missed: {sources})",
          failed: "Balance fetch failed: {reason}",
        },
      },
      tinkoff_retail: {
        title: "T-Bank (retail)",
        description: "Connect to your T-Bank personal account via reverse-API for automatic transaction sync.",
        connect: {
          pin_intro: "You choose this PIN — T-Bank requires it to bind the device. We store it encrypted so subsequent syncs skip the SMS step.",
        },
        relogin: {
          title: "Reconnect",
          password_only_hint: "Session expired. Re-enter your password — phone is preserved.",
        },
        link: {
          title: "Link accounts",
          column: {
            tbank_account: "T-Bank account",
            currency: "Currency",
            local_account: "Local account",
          },
          unlinked_option: "(unlinked)",
          save: "Save links",
          no_external_accounts: "No accounts found. Confirm SMS to fetch the list.",
          manage_button: "Manage links",
          linked_count: "Linked accounts: {count}",
          loading: "Loading accounts from T-Bank…",
          create_and_link: "Create account and link",
          created: "Created and linked",
        },
        error: {
          insufficient_privileges: "Insufficient privileges on this T-Bank account.",
          invalid_credentials: "Invalid phone or PIN.",
          rate_limited: "Too many attempts. Try again later.",
          circuit_open: "Too many errors in a row — sync temporarily halted.",
          unknown: "Could not reach T-Bank.",
          invalid_pin: "PIN must be 4 digits.",
          invalid_phone: "Invalid phone format.",
          captcha_required: "T-Bank showed a captcha. Try later or sign in manually in the app.",
          session_expired: "T-Bank session expired. Reconnect the integration.",
          sms_timeout: "SMS didn't arrive or code wasn't entered in time.",
          no_session: "No saved session — reconnect the integration.",
          no_pending_sms: "Nothing is waiting for an SMS code right now.",
          login_failed: "Could not complete login.",
          unknown_step: "Unknown screen during login. Logs captured — diagnosis needed.",
          sms_input_missing: "SMS code field did not appear. Try again later.",
          pin_screen_missing: "PIN screen did not appear. Try again later.",
          mybank_redirect_missing: "Did not redirect to your account. Try again.",
          push_confirmation_required: "Confirm the login in your T-Bank mobile app.",
          too_many_transitions: "Too many screens during login. Try again later.",
          invalid_otp: "Invalid confirmation code.",
          invalid_password: "Invalid LK password.",
          too_many_attempts: "Too many attempts. Wait and try later.",
          network_error: "Network unreachable. Check your connection and retry.",
          navigation_timeout: "T-Bank is responding slowly. Try again in a minute.",
          browser_unavailable: "The internal browser failed to start. This is on our side.",
          invalid_session: "Saved session is corrupted. Reconnect the integration.",
          lk_password_required: "Enter your T-Bank online banking password.",
          tinkoff_session_cookies_missing: "Failed to extract T-Bank session cookies. Reconnect the integration.",
          create_and_link_failed: "Failed to create account. Try again.",
        },
        action: {
          relogin: "Reconnect",
          manage_links: "Import accounts",
        },
      },
      sync: {
        result: "+{created} / skipped {skipped}",
        ok: "ok",
        last_sync: "sync:",
        in_progress: "Syncing…",
        completion: {
          title_ok: "Sync complete",
          title_error: "Sync finished with errors",
          created: "Created {count}",
          updated: "Updated {count}",
          errors: "Errors {count}",
          go_transactions: "Go to transactions",
          close: "Close",
        },
      },
      cadence: {
        label:   "autosync",
        "1h":    "1h",
        "6h":    "6h",
        "12h":   "12h",
        "24h":   "24h",
        off:     "off",
        next_in: "next in {when}",
      },
    },
    export: {
      section_title: "Data export",
      description: "Download all your data as JSON.",
      button: "Download all (JSON)",
    },
    danger: {
      section_title: "Danger zone",
      wipe_button: "Delete all my data",
      dialog_title: "Confirm deletion",
      dialog_body: "This will delete all your transactions, accounts, loans, subscriptions, funds, income sources and other data. This action is irreversible.",
      confirm_label: "Type \"{phrase}\" to confirm",
      confirm_phrase: "DELETE",
      confirm_button: "Delete all data",
      cancel_button: "Cancel",
      error_wipe: "Error deleting data",
    },
    link: {
      manage: "Manage →",
    },
  },

  summary: {
    settings: {
      mode: "mode",
      modeValue: "normal",
      view: "view",
      viewValue: "settings",
    },
    home: {
      mode_key: "mode",
      period_key: "period",
      period_val: "30d",
      mode_economy: "economy",
      mode_normal: "normal",
      mode_free: "free",
      // === wave-1+2 shell ===
      cashflow: {
        delta_pos: "+{amount} vs prev week",
        delta_neg: "−{amount} vs prev week",
      },
    },
    transactions: {
      filter_label: "filter",
      filter_meta: "30d · all",
      found_key: "found",
      inflow_key: "inflow",
      outflow_key: "outflow",
      avg_day_key: "avg / day",
      mode_key: "mode",
      mode_val: "normal",
      view_key: "view",
      view_val: "feed",
      pending_key: "pending",
      pending_val: "{count} planned",
      pending_none: "none",
      // === wave-1+2 shell ===
      pending_tooltip: "Planned transactions awaiting confirmation",
    },
    income: {
      month_label: "income this month",
      fact_key: "actual",
      upcoming_label: "upcoming income",
      upcoming_meta: "14d window",
      upcoming_none: "no upcoming income",
      mode_key: "mode",
      mode_val: "normal",
      view_key: "view",
      view_val: "income",
      nearest_key: "nearest",
      nearest_none: "none",
    },
    expenses: {
      reserve_label: "reserve 30d",
      loans_key: "loans 30d",
      subs_key: "subs/mo",
      subs_label: "subscriptions · rate/mo",
      personal_key: "personal",
      split_key: "shared",
      paid_for_others_key: "for others",
      total_key: "total",
      mode_key: "mode",
      mode_val: "normal",
      reserve_key: "reserve",
      subs_count_key: "subs",
    },
    planning: {
      funds_label: "savings · total",
      goal_total_key: "total goal",
      progress_key: "progress",
      contrib_month_key: "contrib / mo",
      events_label: "upcoming events",
      events_meta: "14d window",
      events_none: "no events in next 14 days",
      mode_key: "mode",
      mode_val: "normal",
      view_key: "view",
      view_val: "planning",
      nearest_key: "next date",
      nearest_none: "no events",
    },
    analytics: {
      period_label: "period · {period}",
      avg_net_label: "monthly average",
      avg_net_unit: "₽/mo",
      avg_net_sub: "avg net",
      weather_label: "weather · status",
      savings_key: "savings rate",
      status_key: "status",
      delta_label: "top growth · categories",
      no_data: "no data",
      flow_label: "flow · {period}",
      flow_meta: "live",
      mode_key: "mode",
      mode_val: "normal",
      view_key: "view",
      view_val: "analytics",
      weather_key: "weather",
      weather_sun: "sunny",
      weather_cloud: "cloudy",
      weather_rain: "rainy",
      weather_storm: "stormy",
    },
    wallet: {
      net_label: "total balance",
      accounts_sub: "across {n} accounts",
      inst_share_label: "share by institution",
      inst_share_meta: "of total",
      ccy_label: "balances by currency",
      no_accounts: "no active accounts",
      accounts_label: "accounts",
      mode_key: "mode",
      mode_val: "normal",
      view_key: "view",
      view_val: "wallet",
      accounts_key: "accounts",
    },
    family: {
      space_label: "space",
      space_meta: "switch",
      space_shared: "Shared",
      space_personal: "Personal",
      space_hint: "New transactions are created in shared space by default. Change when adding if needed.",
      no_group_label: "group",
      no_group_meta: "not created",
      no_group_hint: "Create a family group on the Family tab to see the shared balance here.",
      mode_key: "mode",
      mode_val: "normal",
      group_key: "group",
      group_none: "none",
    },
    business: {
      label: "business · this mo",
      count_key: "active",
      revenue_key: "revenue",
      profit_key: "profit",
    },
  },

  expenses: {
    loans: {
      section_title: "Loans",
      empty: "no loans yet",
      add: "+ Loan",
      amortization: "amortization",
    },
    long_projects: {},
    kpi: {
      loans: "LOANS",
      loans_sub: "/mo · {count} active",
      subs: "SUBSCRIPTIONS",
      subs_sub: "/mo · {count} active",
      utilities: "UTILITIES + HOME",
      taxes: "TAXES · HINTS",
      taxes_sub: "pay manually",
      projects: "LONG PROJECTS",
      projects_unit: "items",
      projects_sub: "{count_label} · {budget} budget",
      section_title: "obligations overview",
      section_meta: "current month",
    },
    category: {
      usage_label: "{pct}% of total expenses",
      total_of: "of {total}",
      project_category: "category: {name}",
      section_title: "categories",
      section_meta: "current month",
      section_sort: "by amount",
      empty: "no category data",
    },
    badge: {
      personal: "Personal",
      split: "Sharing",
      paid_for_others: "Paying for others",
    },
    interval: {
      monthly: "monthly",
      quarterly: "quarterly",
      semiannual: "semi-annual",
      annual: "annual",
      custom: "every {n} mo",
    },
    subscriptions: {
      pageTitle: "subscriptions",
      manage: "Manage subscriptions",
      totals: {
        personal_label: "personal",
        split_label: "split",
        paid_for_others_label: "paid for others",
      },
      summary: {
        activeCount: "{count} active",
        monthly: "{amount}/mo",
        personalLabel: "personal",
        splitLabel: "shared",
        paidForOthersLabel: "for others",
        addButton: "+ Add",
      },
      group: {
        personal: {
          title: "personal",
          subtitle: "· {count} ·",
          empty: "no personal subscriptions",
        },
        split: {
          title: "shared",
          subtitle: "· {count} ·",
          empty: "nothing shared",
        },
        paidForOthers: {
          title: "for others",
          subtitle: "· {count} ·",
          empty: "not paying for anyone",
        },
      },
      card: {
        nextPayment: "next {date}",
        perMonth: "/mo",
        myShare: "my share {amount}",
        sharedWith: "shared with {count}",
        markPaid: "Mark paid",
        intervalMonthly: "monthly",
        intervalQuarterly: "quarterly",
        intervalYearly: "yearly",
        intervalCustom: "every {n} mo",
        variable_badge: "3-mo avg",
      },
      variable: {
        field_label: "Variable price",
        field_help: "price changes each period",
        price_label_estimate: "Price estimate",
        keywords_caption: "Recognized by",
        keywords_empty: "no aliases",
      },
      history: {
        title: "Payment history",
        empty: "No linked payments",
        source_auto: "auto",
        source_manual: "manual",
        source_cash: "cash",
        unlink: "Unlink",
        unlink_with_rollback: "Unlink & roll back date",
        variance_plus: "+{pct}%",
        variance_minus: "{pct}%",
      },
      suggestions: {
        section_title: "Possible subscription payments",
        reason_alias_ambiguous: "ambiguous alias",
        reason_similarity: "similar to",
        reason_amount_date: "amount + date",
        confirm: "This is {name}",
        dismiss: "Dismiss",
        count_more: "+{n} more",
      },
      duplicates: {
        section_title: "Possible duplicates",
        action_merge: "Merge",
        action_dismiss: "Not duplicates",
      },
      merge: {
        dialog_title: "Merge subscriptions",
        pick_keep_label: "Which subscription to keep?",
        confirm_button: "Merge",
        warning_archived: "The other subscription will be archived and its payments moved to the kept one.",
        error_generic: "Error — please try again",
        action_merge: "Merge",
        action_not_duplicate: "Not duplicates",
        clear_selection: "Clear",
        selected_count: "Selected: {count}",
        bar_label: "Selected subscriptions: {count}",
        need_two_hint: "Select exactly 2 subscriptions",
        select_label: "Select",
        deselect_label: "Deselect",
      },
      reimbursement: {
        section_title: "Possible reimbursements",
        reason_amount_match: "amount match",
        reason_amount_and_name_match: "amount + name",
        confirm: "This is a refund for {name}",
        dismiss: "Dismiss",
        no_spend_hint: "No spend to link yet",
        income_label: "Income",
        spend_label: "Spend",
        sub_label: "Subscription",
        from_label: "from {name}",
      },
      pay: {
        mode_link: "Link a charge",
        mode_manual: "Record manually",
        link_title: "Select a transaction",
        link_empty: "No matching transactions",
        link_confirm: "Link",
      },
      import: {
        button: "Import",
        title: "Import subscriptions",
        hint: "JSON array of subscription objects. See sample below.",
        sample_label: "Sample",
        submit: "Upload",
        success: "Created: {n}",
        error_row: "Row {index}: {message}",
        parse_error: "Invalid JSON",
      },
      json: {
        button: "JSON",
        title: "Subscriptions · JSON",
        empty_hint: "List is empty — example below.",
        valid: "Valid",
        parse_error: "JSON error: {message}",
        schema_error: "Schema error: {path} — {message}",
        diff_summary: "Create: {created} · Update: {updated} · Delete: {deleted}",
        confirm_apply: "Apply?",
        cancel: "Cancel",
        apply: "Apply",
        applying: "Applying…",
      },
      fromSelection: {
        dialog_title: "Subscription from transactions",
        mode_create: "Create new",
        mode_link: "Link to existing",
        field_name: "Name",
        field_name_placeholder: "e.g. Netflix",
        field_variable_price: "Variable price",
        field_interval: "Billing interval",
        field_interval_1: "Monthly",
        field_interval_3: "Quarterly",
        field_interval_6: "Semi-annual",
        field_interval_12: "Annual",
        field_currency: "Currency",
        submit_create: "Create & link",
        submit_link: "Link",
        select_subscription: "Select subscription",
        loading_subscriptions: "Loading…",
        no_subscriptions: "No active subscriptions",
        selected_count: "{count} txn(s)",
        error_internal: "Error — please try again",
        error_conflict: "One of the transactions is already linked",
        error_not_found: "Object not found",
      },
    },
    filter: {
      label_section: "SECTION",
      label_period: "PERIOD",
      section_all: "All",
      section_loans: "Loans",
      section_subs: "Subscriptions",
      section_projects: "Projects",
      section_taxes: "Taxes",
      period_30d: "30d",
      period_90d: "90d",
      period_1y: "1y",
      period_all: "all",
    },
    credit_cards: {
      title: "Credit cards",
      tag: "CARD",
      min_payment: "Minimum payment",
      due_in_days: "In {days}d",
      due_today: "Today",
      overdue: "Overdue",
      pay_button: "Pay",
      debt_label: "Debt",
      empty: "no credit cards with obligations",
    },
  },

  income: {
    kpi: {
      ytd_sub: "{months} mo · avg {avg} / mo",
      sources_sub_work: "{work} employment · {freelance} freelance",
      sources_empty: "no sources",
      tax_sub: "hints only · manual",
      rate_sub_primary: "from primary source",
      rate_sub_empty: "no data",
      inflow_label: {
        "30d": "Last 30 days",
        "90d": "Last 90 days",
        "1y": "Last 365 days",
        all: "All time",
      },
      inflow_sub: "{days} d · avg {avg} / mo",
      sources_label: "ACTIVE SOURCES",
      tax_label: "TAX ACCRUED",
      rate_label: "HOURLY RATE",
      // === wave-3 polish ===
      empty_cta_title: "Add a work source",
      empty_cta_body: "To see tax, hourly rate, and the hours-to-purchase calculation.",
      empty_cta_button: "+ Add work source",
      sources_hint_title: "income exists but sources not configured",
      sources_hint_body: "Add a work source to see tax hints, hourly rate, and hours-to-purchase calculation.",
      section_title: "income",
    },
    work_sources: {
      add: "+ Add source",
      empty: "No sources yet — add a job or freelance source",
    },
    sources: {
      comparison: {
        title: "Source comparison",
        empty: "Less than two sources — nothing to compare",
      },
    },
    work: {
      kind_label: {
        employment: "Job",
        freelance: "Freelance",
        one_time: "One-time",
        source: "Source",
      },
      tax: {
        ndfl: "NDFL {pct}%",
        szn: "Self-emp. {pct}%",
        none: "No tax",
      },
      card: {
        last_payment: "Last payment",
        mtd_total: "MTD total",
        typical_monthly: "Typical (3-month avg)",
        next_expected: "Next expected",
        no_payments: "No payments yet",
        tax_label: "TAX",
      },
      detail: {
        back: "All sources",
        edit: "Edit",
        deactivate: "Deactivate",
        activate: "Activate",
        period: {
          "1m": "1m",
          "3m": "3m",
          "6m": "6m",
          "12m": "12m",
          all: "All",
        },
        kpi: {
          total_income: "Income (period)",
          txn_count: "Transactions",
          avg_per_month: "Avg per month",
          effective_rate: "Effective hourly rate",
          estimated_tax: "Estimated tax",
          tax_hint: "Σ income × {pct}%, ignores what's actually paid",
          avg_days_to_pay: "Avg time to pay",
          late_streak: "Late streak",
          days_unit: "days",
        },
        forecast: {
          title: "Upcoming expected payments",
          note: "Synthetic forecast (not in DB)",
        },
        chart: {
          monthly: "Monthly",
          cumulative: "Cumulative",
          empty: "No data for the selected period",
        },
        txns: {
          title: "Transactions",
          filter: {
            all: "All",
            done: "Done",
            planned: "Planned",
            partial: "Partial",
          },
          empty: "No transactions linked",
          more: "+ {count} more",
        },
        orders: {
          title: "Orders",
          empty: "No orders in this period",
          paid_prefix: "Paid",
          tips_suffix: "tips",
          per_hour: "/h",
          add: "Add order",
          paid_sum_line: "Received {paid} / {total} · {count} payments",
          payments_toggle: "Payments",
          payments_empty: "No linked transactions",
          different_ccy_excluded: "{count} payment(s) in another currency — not counted",
          plan_fact: "received {received} / plan {plan} · remaining {remaining}",
          open_full: "Open",
          stages_title: "Stages",
          implicit_stage: "Single payment",
          status: {
            planned: "Planned",
            active: "Active",
            awaiting_payment: "Awaiting payment",
            completed: "Completed",
            cancelled: "Cancelled",
          },
          grid: {
            view: {
              cards: "Cards",
              table: "Table",
            },
            col: {
              title: "Title",
              client: "Client",
              status: "Status",
              amount: "Amount",
              currency: "Currency",
              hours: "Hours",
              rate: "Rate",
              tips: "Tips",
              performed: "Performed",
              paid: "Paid",
              note: "Note",
              received: "Received",
              remaining: "Remaining",
              actions: "",
            },
            add_row: "+ row",
            edit_hint: "Click a cell to edit",
          },
        },
        status_breakdown: {
          title: "Order statuses",
          empty: "No orders in period",
        },
        status: {
          done: "Done",
          planned: "Planned",
          partial: "Partial",
          missed: "Missed",
          cancelled: "Cancelled",
        },
        plan: {
          title: "Plan vs fact (monthly)",
          column: {
            month: "Month",
            plan: "Plan",
            fact: "Fact",
            delta: "Δ",
          },
        },
        lifecycle: {
          started: "From {date}",
          ended: "Until {date}",
          archived: "Archived",
        },
        currency: {
          base_annotation: "≈ {amount} {ccy}",
        },
        not_found: "Source not found",
      },
    },
    expected: {
      status_label: "Expected",
      empty: "No planned inflows yet",
      events_count: "{count} events in the next 90 days",
      add: "+ Plan inflow",
    },
    freelance_orders: {
      section_title: "Orders",
      empty: "No orders yet",
      add: "+ Order",
      link_txn: "Link transaction",
      unlink_txn: "Unlink",
      link_picker: {
        title: "Available transactions",
        empty: "No unlinked transactions for this source",
      },
    },
    order_detail: {
      heading: "Order",
      description_label: "Description",
      payments_history: "Payment history",
      hours_rate: "Hours / Rate",
      dates: "Dates",
      notes: "Notes",
      delete: "Delete order",
      delete_confirm: "Delete this order? This cannot be undone.",
      delete_confirm_title: "Delete order",
      delete_confirm_body: "Delete \"{name}\"? This cannot be undone.",
      back: "Back to source",
      empty_stages: "No stages — order is paid as a single payment",
      edit: "Edit",
      status_label: "STATUS",
    },
    today: "today",
    in_days: "+{n}d",
    filter: {
      label_view: "VIEW",
      label_period: "PERIOD",
      view_sources: "Sources",
      view_expected: "Expected",
      view_other: "Other",
      period_30d: "30d",
      period_90d: "90d",
      period_1y: "1y",
      period_all: "all",
      day_of_month_hint: "day of month / days in month",
    },
  },

  business: {
    index: {
      title: "Business",
      section_title: "Ventures",
      add: "+ Business",
      empty: "No businesses yet — add the first one to track P&L separately from personal finances",
    },
    card: {
      inactive: "inactive",
      period_revenue: "revenue/mo",
      period_profit: "profit/mo",
      txn_count: "{n} transactions",
    },
    detail: {
      add_entry: "+ entry",
      edit: "edit",
      empty_txns: "No transactions linked to this business yet. Attribute an existing transaction or add a new entry.",
      period: {
        "1m": "1m",
        "3m": "3m",
        "6m": "6m",
        "12m": "12m",
        all: "all",
      },
      kpi: {
        section_title: "P&L",
        revenue: "revenue",
        revenue_sub: "business income, excluding pass-through amounts",
        expenses: "expenses",
        expenses_sub: "all expenses linked to this business",
        profit: "profit",
        profit_sub: "revenue minus expenses",
        cumulative: "cumulative profit",
        cumulative_sub: "running total over the period",
        pass_through_note: "Pass-through income (received and forwarded on) is excluded from revenue — shown in a separate column in the table below",
      },
      matrix: {
        title: "Monthly P&L",
        column: {
          month: "month",
          revenue: "revenue",
          pass_through: "pass-through",
          expenses: "expenses",
          profit: "profit",
          cumulative: "cumulative",
        },
      },
      stream_matrix: {
        title: "Revenue by stream",
        column: {
          stream: "stream",
          total: "total",
        },
      },
      tariff_breakdown: {
        title: "Revenue by tariff",
        column: {
          tariff: "tariff",
          total: "total",
          count: "entries",
        },
      },
      charts: {
        stream: {
          title: "Revenue by stream",
          empty: "No stream data for this period",
        },
        pnl: {
          title: "Monthly revenue / expenses",
          legend_revenue: "revenue",
          legend_expenses: "expenses",
          legend_cumulative: "cumulative profit",
        },
        tariff: {
          title: "Tariffs",
          empty: "No tariff data for this period",
        },
        forecast: {
          title: "6-month forecast",
          legend_history: "actual",
          legend_expenses: "expenses",
          scenario_base: "base",
          scenario_optimist: "optimistic",
          scenario_pessimist: "pessimistic",
          empty: "Not enough history to forecast — at least 2 months of data required",
        },
      },
    },
    allocation: {
      field: {
        entry_type: "entry type",
        amount: "amount",
        currency: "currency",
        occurred_at: "date",
        stream_key: "stream",
        tariff: "tariff",
        note: "note",
      },
      entry_type: {
        revenue: "revenue",
        pass_through: "pass-through",
        expense: "expense",
      },
      stream_key_hint: "e.g. bot, arcadia, zakhar — free text",
      error: {
        over_allocated: "Allocation total exceeds the transaction amount",
      },
      offapp: {
        title: "Off-app revenue",
        entry_cta: "+ off-app revenue",
        hint: "Not linked to a transaction — doesn't affect personal balance, only business P&L",
      },
      split: {
        title: "Split transaction",
        txn_amount_hint: "Transaction amount: {amount}",
        remainder: "Stays personal: {amount}",
        add_row: "+ row",
        remove_row: "remove",
        empty_error: "Add at least one amount",
        entry_placeholder: "Transaction ID",
        entry_cta: "split",
      },
    },
    stream: {
      bot: "bot",
      arcadia: "Arcadia",
      zakhar: "Zakhar",
      manual: "manual",
      esp: "ESP",
      evo: "EVO",
      other: "other",
    },
    tariff: {
      lite: "lite",
      standard: "standard",
      pro: "pro",
      premium: "premium",
      other: "other",
    },
  },

  planning: {
    funds: {
      section_title: "Funds",
    },
    status_strip: {
      view_label:    "VIEW",
      horizon_label: "HORIZON",
      view_all:      "All",
      view_calendar: "Calendar",
      view_funds:    "Savings",
      view_buys:     "Purchases",
      view_trips:    "Trips",
      horizon_30d:   "30d",
      horizon_90d:   "90d",
      horizon_1y:    "1y",
      horizon_all:   "all",
      day_hint:      "day of month / days in month",
    },
    kpi: {
      saved: "SAVED",
      saved_sub: "{count} {word}",
      monthly: "CONTRIB / MO",
      monthly_sub: "total contribution",
      next_event: "NEXT EVENT",
      next_event_none: "no events",
      hours: "HOURS TO GOAL",
      hours_no_rate: "rate not set",
      section_title: "future plan",
      section_subtitle: "overview",
      funds_count: "{n} {word}",
    },
    fund_kind: {
      TRIP: "trip",
      BUY: "purchase",
      VAULT: "cushion",
      GIFT: "gift",
      OTHER: "goal",
    },
    event_kind_letter: {
      BIRTHDAY: "B",
      HOLIDAY: "H",
      TRIP: "T",
      PURCHASE: "P",
      OTHER: "O",
    },
    purchase_icon: {
      BUY: "P",
      OTHER: "O",
    },
    fund_stat: {
      saved: "saved",
      goal: "goal",
      monthly_contrib: "contrib / mo",
      months_left: "months left",
    },
    calendar: {
      section_title: "Events",
      events_count: "{n} {word}",
      in_days: " · {n}d",
    },
    upcoming: {
      event_fallback: "event",
    },
    deadline_none: "no deadline",
    hours_rate_label: "rate {rate}",
    // === wave-3 polish ===
    hours_calc: {
      title: "hours calculator",
      subtitle: "how much work a purchase costs",
      source_meta: "source: {name} · {rate}/h (net)",
      price_label: "purchase price",
      price_hint: "enter amount and currency",
      rate_label: "rate / hour (net)",
      rate_sub: "{name} · {hpm} h/mo · {tax}",
      hours_label: "net hours of work",
      hours_sub: "≈ {weeks} weeks at your schedule",
      empty_title: "no rate set",
      empty_body: "Add a work source to see how many hours a purchase costs.",
      empty_cta: "+ Add work source",
    },
    upcoming_dates: {
      title: "upcoming dates",
      subtitle: "auto-suggest saving",
      meta: "{count} {word}",
      empty: "no upcoming events",
    },
    timeline: {
      title: "event feed",
      subtitle: "planning horizon",
      empty_title: "no events in period",
      empty_body: "Add events, subscriptions, loans or savings goals to see them here.",
      today: "today",
      legend: {
        trip: "trips",
        event: "events",
        subscription: "subscriptions",
        loan: "loans",
        fund_target: "goals",
        txn_planned: "planned",
      },
      kind: {
        subscription: "subscription",
        loan_payment: "payment",
        fund_target: "savings goal",
        txn_planned: "planned txn",
      },
    },
    mini_calendar: {
      title: "calendar",
      subtitle: "two months",
      click_hint: "click a day",
    },
    calendar_page: {
      h1: "Calendar",
      sub: "visual event timeline",
      breadcrumb_planning: "Planning",
      breadcrumb_calendar: "Calendar",
    },
    trips: {
      list: {
        title: "Trips",
        subtitle: "travel and journeys",
        empty_title: "no trips yet",
        empty_body: "Create your first trip — add a budget and track expenses.",
        create_cta: "+ Trip",
        archived_section: "Archive",
      },
      card: {
        date_range: "{start} — {end}",
        days_to_start: "in {n} d",
        in_trip: "on trip",
        past: "past",
        status: {
          draft: "draft",
          planning: "planning",
          active: "active",
          archived: "archived",
        },
        budget_label: "budget",
        fund_linked: "fund linked",
      },
      create: {
        title: "New Trip",
        name_label: "name",
        destination_label: "destination",
        start_label: "start",
        end_label: "end",
        currency_label: "currency",
        budget_label: "budget",
        fund_section_title: "savings fund",
        fund_none: "no fund",
        fund_existing: "link existing fund",
        fund_required_monthly: "contrib / mo (calculated)",
        submit: "Create",
        cancel: "Cancel",
        validation: {
          dates_inverted: "start date is after end date",
          budget_positive: "budget must be > 0",
        },
      },
      detail: {
        breadcrumb: "Trips",
        countdown_days: "in {n} d",
        countdown_today: "starts today",
        in_trip: "on trip",
        past: "past",
        hero_hours_badge: "{n} h of work",
        edit: "Edit",
        delete: "Delete",
        delete_confirm: "Delete trip? This cannot be undone.",
      },
      savings: {
        title: "savings",
        current: "saved",
        goal: "goal",
        required_monthly: "contrib / mo",
        deadline_label: "by trip start",
        no_fund_hint: "Link a savings fund to this trip.",
        link_fund_cta: "Link Fund",
        topup_fund_cta: "Top Up",
      },
      rollup: {
        title: "categories",
        kind: {
          transport: "Transport",
          lodging: "Lodging",
          food: "Food",
          activity: "Activities",
          other: "Other",
        },
        col_allocation: "plan",
        col_spent: "spent",
        col_remaining: "remaining",
        edit_allocation: "edit",
        save_allocation: "save",
        mixed_ccy_tooltip: "mixed currencies, converted to {ccy}",
      },
      bookings: {
        title: "bookings",
        add_row: "+ Add",
        col_kind: "type",
        col_label: "name",
        col_date: "date",
        col_amount: "amount",
        col_status: "status",
        col_actions: "",
        status: {
          planned: "planned",
          paid: "paid",
        },
        mark_paid: "Pay",
        mark_paid_modal_title: "Log as expense",
        mark_paid_account_label: "account",
        mark_paid_submit: "Confirm",
        edit: "Edit",
        delete: "Del.",
        delete_confirm: "Delete booking?",
        empty: "no bookings",
      },
    },
  },

  analytics: {
    kpi: {
      income_period: "INCOME {period}",
      income_sub: "total income",
      expense_period: "EXPENSE {period}",
      expense_sub: "total expenses",
      net_period: "NET {period}",
      net_sub: "net flow",
      net_sub_pos: "positive",
      net_sub_neg: "negative",
      savings_rate: "rate {pct}%",
      no_data: "no data",
      avg_spend_day: "AVG SPEND / DAY",
      avg_spend_day_sub: "average",
      // === wave-1+2 shell ===
      title: "key metrics",
      title_period: "· {period}",
      period_default: "current period",
    },
    forecast: {
      income: "EXP. INCOME",
      expense: "EXP. EXPENSE",
      net: "EXP. NET",
      sub_plan: "plan + actual month",
      net_pos: "positive",
      net_neg: "negative",
      title: "forecast",
      subtitle: "simplified · end of period",
      meta: "no pessimistic scenario · see vision",
      empty: "no forecast data",
      year_net: "EXP. NET / YEAR",
      year_sub: "trailing-average extrapolation",
      year_low_confidence: "limited history",
      reserved: "RESERVED",
      free: "FREE",
      liquid: "LIQUID",
      posture_sub: "as of now",
    },
    prescriptive: {
      title: "what you can do",
      subtitle: "burn rate · where to cut",
      meta: "diagnostic → suggestion",
      burn: {
        title: "burn rate",
        per_day_30: "/DAY · 30D",
        per_day_90: "/DAY · 90D",
        days_to_zero: "DAYS TO ZERO",
        days_to_zero_sub: "at current rate",
        already_negative: "no free funds",
        no_burn: "no spending",
      },
      shrink: {
        title: "where to cut",
        subtitle: "above 6-month norm",
        col_current: "current",
        col_avg: "norm",
        col_over: "overspend",
        empty: "all categories within norm",
      },
      split: {
        title: "essential vs discretionary",
        subtitle: "where you can cut",
        obligatory: "essential",
        discretionary: "discretionary",
        cuttable_pct: "can cut",
        empty: "no spending in period",
      },
      economy_exit: {
        title: "exit from deficit",
        subtitle: "Economy mode · months to recover",
        months: "≈ {n} mo in Economy",
        no_deficit: "already in surplus",
        no_surplus: "Economy mode yields no surplus",
        deficit_label: "deficit",
        recovery_label: "surplus / month",
      },
    },
    mode: {
      ECONOMY: "Economy",
      NORMAL: "Normal",
      FREE: "Free",
      economy_tag: "strict limits · savings",
      normal_tag: "default · all good",
      free_tag: "soft limits",
      limit_key: "overall limit",
      limit_lean: "reduced",
      limit_normal: "standard",
      limit_free: "expanded",
      reference: {
        title: "modes · reference",
        subtitle: "view only · switch on home",
        active_on: "active: {name}",
        active_off: "not set",
        pill_on: "active",
        pill_off: "off",
        safe_until_label: "safe until",
        empty: "modes not configured",
      },
    },
    runway: {
      title: "runway by mode",
      until: "until {date}",
      days_remaining: "{n} days", // TODO: add full plural
      empty: {
        no_limits: "no limits set for this mode",
        cta_set_limits: "set limits",
      },
      breakdown: {
        top_categories: "top categories",
      },
    },
    trends: {
      title: "trend",
      subtitle: "income / expense / net",
      empty: "No data for period",
      legend: {
        income:  "income",
        expense: "expense",
        net:     "net",
      },
      y_axis_label: "k",
      curve: {
        approx: "approximation",
        interp: "interpolation",
        title:  "graph smoothing type",
      },
    },
    status_strip: {
      period_label:  "PERIOD",
      compare_label: "COMPARE TO",
      cmp: {
        prev: "prev. period",
        yoy:  "year ago",
        none: "none",
      },
      day_hint: "day of month / days in month",
    },
    period: {
      calendar: {
        month:     "month",
        quarter:   "quarter",
        year:      "year",
        pick:      "period",
        title:     "pick period",
        prev_year: "previous year",
        next_year: "next year",
      },
    },
    compare: {
      label_prev: "vs previous period",
      label_yoy: "year over year",
      label_none: "no comparison",
      range_caption: "{currentRange} vs {previousRange}",
      title: "comparison with previous period",
      disabled: "comparison disabled",
      no_data_short: "no data",
      empty: "no data for comparison",
      summary: {
        rising: "{count} {word} rising",
        falling: "{count} {word} falling",
      },
      col: {
        category: "Category",
        previous_default: "previous",
        current_default: "current",
        trend_6m: "6m trend",
      },
      delta_new: "new",
      delta_gone: "gone",
    },
    weather: {
      title: "financial weather",
      subtitle: "financial health",
      meta: "recalculated hourly · leading indicator",
      status_label: "status",
      scale_label: "health scale",
      classification_label: "classification",
      kind: {
        sun: "Sunny",
        cloud: "Cloudy",
        rain: "Rain",
        storm: "Storm",
      },
      reserve: "resilience — {n}/10",
      alert_level: "alert level — {score}/10",
      matches: "corresponds to «{label}»",
      formula: "Weather is calculated from savings rate and income/expense balance for the selected period.",
      reason_prefix: "Assessment reason: {hint}.",
      reason: {
        outflow_gt_inflow_period: "expenses exceed income for the selected period",
        savings_rate_lt_5pct: "savings rate below 5%",
        savings_rate_5_to_20pct: "savings rate 5–20%",
        savings_rate_gt_20pct: "savings rate above 20%",
      },
      savings_rate: {
        line: "savings rate {value}% for the selected period",
        no_data: "insufficient data for the selected period",
      },
      classification: {
        sun: "full resilience",
        cloud: "all good, minor signals",
        rain: "one factor is dipping",
        storm: "crisis",
        sun_threshold: "rate > 20%",
        cloud_threshold: "5–20%",
        rain_threshold: "< 5%",
        storm_threshold: "expenses > income for the period",
      },
    },
    category_pie: {
      title: "expense categories",
      period_default: "current month",
      meta: "{count} {word} · sorted by amount",
      total_label: "TOTAL",
      legend_period: "period {period}",
      legend_period_default: "current period",
      empty: "no category data",
    },
  },

  family: {
    members: {
      section_title: "Members",
    },
    role: {
      owner: "owner",
      member: "member",
    },
    fund: {
      no_deadline: "no deadline",
      contrib_total: "total",
      progress_label: "{pct}% · of ₽ {total}",
    },
    sub: {
      sharing: "Sharing",
      paid_for_others: "Paying for others",
    },
    member: {
      balance_key: "balance",
    },
    space: {
      personal_tag: "PERSONAL",
      personal_name: "my space",
      personal_sub: "visible only to me",
      personal_balance: "personal balance",
      shared_tag: "SHARED",
      shared_name: "space «{name}»",
      shared_name_default: "shared space",
      shared_sub: "visible to all members",
      shared_balance: "shared balance",
    },
    status_strip: {
      space_label: "SPACE",
      space_shared: "Shared",
      space_personal: "My personal",
      space_all: "All together",
      period_label: "PERIOD",
      period_month: "this mo",
      period_90d: "90d",
      period_ytd: "YTD",
      period_all: "all",
      day_hint: "day of month / days in month",
    },
    your_share: "your share",
    spaces: {
      title: "spaces",
      subtitle: "personal / shared",
      hint: "switch when adding a transaction",
      empty: "no spaces",
    },
    balances: {
      title: "reconciliation",
      subtitle: "who owes whom",
      settle_btn: "Settle payment",
      empty: "no current debts",
      algo_hint: "algorithm: minimum number of transfers to zero out all differences",
    },
    shared_funds: {
      title: "shared savings",
      subtitle: "who contributed what",
      meta: "{count} {word}",
      empty: "no shared savings",
      progress_key: "progress",
    },
  },

  transactions: {
    debt_meta: "{count} active · net",
    toolbar: {
      import_csv: "CSV Import",
      import_csv_title: "Import transactions from CSV",
      // === wave-3 polish ===
      search_placeholder: "search by description",
    },
    sync: {
      button: "Sync",
      last: "upd {ago}",
      never: "never",
      in_progress: "syncing…",
      error: "error",
      no_credentials: "Connect a bank in settings",
      toast_no_credentials: "No connected integrations",
      toast_adapter_ok: "{name}: {count} new",
      toast_adapter_synced: "{name}: synced",
      toast_adapter_err: "{name}: error",
      toast_already_running: "Already running",
      toast: {
        launched: "Sync started",
        timeout: "Sync timed out",
      },
      button_no_integrations: "no integrations",
      button_syncing: "syncing…",
      button_relative: "synced {when}",
    },
    feed: {
      title: "feed",
      meta: "newest first",
      shown_of: "{shown} of {total}",
      load_more: "Load more",
      daily_net: "daily net",
    },
    new: {
      no_accounts: "No accounts found. Create an account in Wallet first.",
    },
    filter: {
      label_type: "TYPE",
      label_period: "PERIOD",
      type_all: "All",
      type_inc: "Income",
      type_exp: "Expenses",
      type_xfr: "Transfers",
      type_loan: "Loans",
      period_7d: "7d",
      period_30d: "30d",
      period_90d: "90d",
      period_1y: "1y",
    },
    period: {
      title: "period summary",
      range: "· {period}",
      txn_short: "txn.",
      avg: "avg",
      net_forecast: "forecast {amount}",
      net_deficit: "deficit {amount}",
      net_zero: "net 0",
      planned_count: "{n} planned",
      partial_count: "{n} partial",
      column: {
        inflow: "INFLOW",
        outflow: "OUTFLOW",
        xfr: "TRANSFERS",
        net: "NET",
      },
    },
    status: {
      planned: "Planned",
      partial: "Partial",
      done: "Done",
      missed: "Missed",
      cancel: "Cancelled",
    },
    weekday: {
      sun: "Sun",
      mon: "Mon",
      tue: "Tue",
      wed: "Wed",
      thu: "Thu",
      fri: "Fri",
      sat: "Sat",
    },
    day: {
      today: "· today",
      inflow: "inflow",
      outflow: "outflow",
      planned: "planned",
      expected: "{n} expected",
      cancelled: "{n} cancelled",
    },
    selection: {
      count: "selected: {count}",
      aria_select_row: "Select transaction",
      mark_as_transfer: "Transfer",
      break_transfer: "Break",
      mark_as_compensation: "Compensation",
      clear: "Clear",
      tooltip: {
        need_two: "Select exactly 2 rows on different accounts",
        same_account: "Both rows are on the same account",
        already_transfer: "These rows are already part of a transfer",
        already_grouped: "One of the transactions is already grouped",
        need_mixed_kinds: "Select ≥1 expense and ≥1 income",
        bad_status: "Only confirmed transactions can be grouped",
        scope_mismatch: "All transactions must share the same scope",
        break_link: "Break link (transfer or compensation)",
        break_mixed: "Selection contains both transfers and compensations — break one type at a time",
      },
      error: {
        same_kind: "Cannot mark two rows of the same direction as a transfer",
        rate_missing: "FX rate unavailable — transfer created at rate 1, edit manually",
        transfer_not_found: "Transfer not found or already deleted",
        compensation_failed: "Could not group",
      },
      success: {
        marked: "Marked as transfer",
        broken: "Transfer broken",
        compensation_made: "Compensated",
        compensation_broken: "Compensation removed",
        subscription_created: "Subscription created and linked",
        subscription_linked: "Linked to subscription",
      },
      subscription: {
        action: "Subscription",
        tooltip_need_expense: "Select at least 1 unlinked expense",
        tooltip_already_linked: "One of the transactions is already linked to a subscription",
      },
    },
    compensation: {
      error: {
        too_few: "At least 2 transactions are required",
        same_kind_only: "Both an expense and an income are required",
        bad_status: "Only confirmed transactions",
        scope_mismatch: "Scopes do not match",
        fx_rate_missing: "No FX rate available",
        edit_blocked: "Break the compensation to edit",
        invalid_group: "invalid group",
      },
      badge: {
        label: "comp · {sign}{n}",
      },
      tooltip: "compensation (+{n})",
      dialog: {
        title: "Compensation group",
        netto_label: "netto",
        members_count: "members: {n}",
        break_button: "Break compensation",
        transfer_label: "Transfer",
      },
    },
    merge: {
      action_label: "Merge",
      group_noun: "Merged",
      badge: "merged · {n}",
      tooltip: {
        need_same_sign: "select same-sign transactions",
      },
      error: {
        need_same_sign: "Merge requires all expenses or all incomes",
        failed: "Could not merge",
      },
      success: {
        merged: "Merged",
        broken: "Merge removed",
      },
      dialog: {
        title: "Merged group",
        sum_label: "sum",
        members_count: "members: {n}",
        break_button: "Split",
      },
    },
    edit_sheet: {
      title_txn: "Edit transaction",
      title_transfer: "Edit transfer",
    },
  },

  wallet: {
    // === wave-2 parser+wallet+breadcrumbs ===
    totals: {
      section_title:     "wallet total",
      section_sub:       "all accounts in ₽",
      rates_updated:     "rates · upd {ago}",
      rates_never:       "rates · no data",
      net_label:         "net total",
      net_sub:           "across {n} accounts",
      banks_crypto_label: "banks & crypto",
      banks_crypto_sub:  "cards & crypto · excl. savings and cash",
      savings_label:     "buffer / savings",
      savings_sub:       "{n} savings accounts",
      cash_label:        "cash",
      cash_sub:          "{n} {word}",
    },
    strip: {
      group: "GROUP",
      currency: "CURRENCY",
      group_all: "All",
      group_banks: "Banks",
      group_crypto: "Crypto",
      group_cash: "Cash",
      group_arch: "Archive",
      currency_all: "All",
      day_progress: "d{day}/{days}",
      day_hint: "day of month / days in month",
    },
    cash_meta: "{locations} {locWord} · {currencies} {curWord}",
    add_account: {
      title: "Add new account",
      sub: "manual entry or integration",
      btn_manual: "Manual entry",
      btn_integration: "Integration",
    },
    kind: {
      card: "Debit",
      credit: "Credit",
      savings: "Savings",
      cash: "Cash",
      crypto: "Exchange",
      loan: "Auto-debit",
    },
    collateral: {
      card: "Card",
      credit: "Credit",
      savings: "Savings",
      cash: "Cash",
      crypto: "Crypto",
      loan: "Service",
    },
    archived: {
      closed: "closed",
      recent: "closed recently",
      months: "closed {n} mo ago",
      years: "closed {n} {word} ago",
    },
    account: {
      refreshed: "upd",
      refreshed_ago: "upd {ago}",
      badge: {
        not_in_analytics: "not in analytics",
      },
      edit: {
        archive: "Archive",
        archive_failed: "Failed to archive account",
        requisites_section: "Banking requisites",
      },
      credit: {
        debt: "Debt",
        debt_inline: "Debt: {amount}",
        available: "Available",
        limit: "Limit",
        no_limit: "No limit set",
      },
      form: {
        include_in_analytics: {
          label: "Include in analytics",
          hint: "Affects safe-until, available amount and aggregates",
        },
        credit: {
          rate: "Annual rate (%)",
          limit: "Credit limit",
          grace_period: "Grace period (days)",
          statement_day: "Statement day (1–31)",
          min_payment: {
            label: "Minimum payment",
            percent: "Percentage of debt (%)",
            fixed: "Fixed amount",
            type_percent: "Percent",
            type_fixed: "Fixed",
          },
        },
        savings: {
          rate: "Annual rate (%)",
          capitalization: "Capitalization",
          withdrawal_limit: "Withdrawal limit (optional)",
          cap_none: "No capitalization",
          cap_monthly: "Monthly",
          cap_quarterly: "Quarterly",
          cap_yearly: "Yearly",
        },
      },
      requisites: {
        title: "Requisites",
        account_number: "Account number",
        bic: "BIC",
        bank_name: "Bank",
        inn: "INN",
        kpp: "KPP",
        corr_account: "Correspondent account",
        copy: "Copy",
        copied: "Copied",
        pull_from_tbank: "Pull from T-Bank",
        pulling: "Loading...",
        pull_failed: "Failed to pull requisites",
        payment_due_day: "Payment due day",
        recipient_name: "Recipient",
      },
    },
    cash: {
      section_title: "cash",
      section_sub: "· various places and currencies",
      add_location: {
        title: "New cash location",
        placeholder_location: "Location name (e.g. «home safe»)",
        placeholder_balance: "Starting balance",
        submit: "Add",
        cancel: "Cancel",
      },
      edit: {
        title: "Edit cash location",
        archive: "Archive",
        archive_failed: "Failed to archive account",
        save: "Save",
        cancel: "Cancel",
      },
    },
    fx: {
      add_pair: {
        button: "+ add rate",
        title: "Add currency pair",
        from_label: "From",
        to_label: "To",
        submit: "Add",
        cancel: "Cancel",
        unsupported: "Currency not supported by CBR",
        same_currency_error: "Please select different currencies",
        duplicate_error: "This pair is already added",
      },
      remove_pair: {
        title: "Remove pair",
      },
      section_title: "exchange rates",
      section_sub: "official · for revaluation",
      refresh: "Refresh rates",
      unavailable: "—",
      fetched_ago: "updated {n} min. ago",
      source: {
        cbr: "CBR",
        db: "from cache",
      },
      updated_label: "updated {ago}",
      stale_warning: "may be stale",
    },
  },

  forms: {
    common: {
      save: "Save",
      cancel: "Cancel",
      required: "Required field",
      required_hint: "* — required fields",
      invalid_amount: "Invalid amount",
      invalid_date: "Invalid date",
      loading: "Loading...",
      delete: "Delete",
      archive: "Archive",
      restore: "Restore",
      add: "Add",
      edit: "Edit",
      form_error: {
        internal: "Internal error. Please try again.",
        unique_violation: "A record with this data already exists.",
        not_found: "Record not found.",
        conflict: "Conflict: operation not allowed.",
      },
      kind: {
        income: "Income",
        expense: "Expense",
        transfer: "Transfer",
        loan_payment: "Loan payment",
        debt_out: "Lent out",
        debt_in: "Received / returned",
      },
      scope: {
        personal: "Personal",
        shared: "Shared",
      },
      status: {
        planned: "Planned",
        partial: "Partially done",
        done: "Done",
        missed: "Missed",
        cancelled: "Cancelled",
      },
    },
    tx: {
      title_create: "New transaction",
      title_edit: "Edit transaction",
      title_income: "New income",
      title_expense: "New expense",
      field: {
        kind: "Type",
        account: "Account",
        fromAccount: "From account",
        toAccount: "To account",
        category: "Category",
        amount: "Amount",
        currency: "Currency",
        occurred_at: "Date",
        planned_at: "Planned date",
        status: "Status",
        name: "Name",
        note: "Note",
        scope: "Scope",
        loan: "Loan",
        subscription: "Subscription",
        fund: "Fund",
        long_project: "Project",
        work_source: "Work source",
        work_source_required: "Income source *",
        planned_event: "Event",
        personal_debt: "Debt",
        business: "Business",
        business_entry_type: "Entry type",
      },
      placeholder: {
        name: "Transaction name",
        note: "Note (optional)",
        category: "— select —",
        account: "— select account —",
        currency: "— currency —",
      },
      errors: {
        transfer_same_account: "Source and destination accounts must differ",
        transfer_currency_mismatch: "Cross-currency transfers are not yet supported",
        work_source_required: "Pick an income source",
      },
      work_source: {
        empty_cta_title: "Add an income source first",
        empty_cta_body: "Every income must be tied to a source. Create at least one.",
        empty_cta_button: "Add source",
      },
      business_entry_type: {
        revenue: "Revenue",
        pass_through: "Pass-through",
      },
      hint: {
        business_entry_type: "Pass-through — money received to be forwarded on (excluded from business revenue and profit)",
      },
    },
    transfer: {
      title_create: "New transfer",
      title_edit: "Edit transfer",
      field: {
        from_account: "From account",
        to_account: "To account",
        from_amount: "Debit amount",
        to_amount: "Credit amount",
        rate: "Exchange rate",
        fee: "Fee",
        occurred_at: "Date",
        note: "Note",
      },
      placeholder: {
        from_account: "— from —",
        to_account: "— to —",
        note: "Note (optional)",
        rate: "auto",
      },
      same_account_error: "Source and destination accounts must differ",
      cross_currency_hint: "Different currencies — specify rate",
    },
    account: {
      title_create: "New account",
      title_edit: "Edit account",
      credit: {
        balance_hint: "Enter the available balance (how much you can still spend). Debt is computed automatically: limit − available.",
      },
      errors: {
        credit_rate_required: "Rate is required for credit accounts",
        credit_limit_required: "Credit limit is required",
        savings_rate_required: "Rate is required for savings accounts",
        cash_goes_through_cash_stash: "Cash is added via the cash section, not through the account form",
        balance_required: "Starting balance is required",
        card_last4_already_bound: "This card is already linked to another account",
      },
      field: {
        institution: "Bank / Institution",
        name: "Account name",
        kind: "Account type",
        currency: "Currency",
        balance: "Starting balance",
        balance_required: "Starting balance *",
        balance_label_credit: "Available balance on card",
        note: "Note",
      },
      kind: {
        card: "Card",
        credit: "Credit card",
        savings: "Savings",
        cash: "Cash",
        crypto: "Crypto",
        loan: "Loan",
      },
      placeholder: {
        institution: "— select or create —",
        name: "Account name",
        note: "Note (optional)",
      },
      new_institution: "+ New institution",
      institution_section: "New institution",
      institution_name: "Name",
      institution_kind: "Type",
      institution_kind_bank: "Bank",
      institution_kind_crypto: "Crypto",
      institution_kind_cash: "Cash",
      cardLast4: {
        label: "Linked cards",
        hint: "Last 4 digits of cards. Used for CSV import.",
        placeholder: "Type 4 digits and press Enter",
        chip_remove_aria: "Remove card {last4}",
        error: {
          format: "Must be exactly 4 digits",
          duplicate: "This card is already added",
        },
      },
      section: {
        requisites: {
          title: "Requisites",
        },
      },
      requisites: {
        accountNumber: {
          label: "Account number",
          hint: "20 digits (for RU)",
        },
        bic: {
          label: "BIC",
          hint: "9-digit bank code",
        },
        bankName: {
          label: "Bank name",
        },
      },
      balance: {
        label: {
          edit: "Current balance",
        },
        hint: {
          edit: "Editing overwrites the balance and updates the last-edited timestamp",
        },
      },
    },
    category: {
      title: "Categories",
      back: "Back to settings",
      field: {
        name: "Name",
        kind: "Type",
        parent: "Parent",
        icon: "Icon",
        color: "Color",
        limit_economy: "Economy limit",
        limit_normal: "Normal limit",
        limit_free: "Free limit",
        limit_economy_pct: "Economy limit, % of 6-mo avg",
        limit_normal_pct: "Normal limit, % of 6-mo avg",
        limit_free_pct: "Free limit, % of 6-mo avg",
      },
      kind: {
        income: "Income",
        expense: "Expense",
      },
      filter_all: "All",
      filter_income: "Income",
      filter_expense: "Expenses",
      filter_archived: "Archived",
      add_button: "+ Add category",
      empty: "No categories",
      archived_badge: "archived",
      essential_label: "essential",
      essential_hint: "not cut when economizing",
      limit_pct_hint: "Empty = global mode multiplier ({pct}%)",
      placeholder: {
        name: "Category name",
        parent: "— no parent —",
        icon: "emoji or code",
        color: "hex or name",
      },
    },
    freelance_order: {
      title_create: "New order",
      title_edit: "Edit order",
      delete: "Delete order",
      delete_confirm: "Delete this order?",
      delete_failed: "Failed to delete",
      field: {
        title: "Order title",
        description: "Description",
        client: "Client",
        amount: "Amount",
        hours: "Hours",
        hourly_rate: "Hourly rate",
        tips: "Tips",
        status: "Status",
        performed_at: "Performed",
        paid_at: "Paid",
        note: "Note",
        rate_mode: "Rate type",
      },
      rate_mode: {
        hourly: "Hourly",
        fixed: "Fixed",
      },
      placeholder: {
        title: "Order title",
        description: "Order description (optional)",
        client: "Client name",
        note: "Note (optional)",
      },
      status: {
        planned: "Planned",
        active: "Active",
        awaiting_payment: "Awaiting payment",
        completed: "Completed",
        cancelled: "Cancelled",
      },
      hint: {
        hours_info: "Hours — informational, do not affect amount",
        rate_info: "Hourly rate — informational",
      },
    },
    freelance_order_stage: {
      field: {
        label: "Stage name",
        expected: "Expected amount",
        due_date: "Due date",
      },
      status: {
        pending: "Pending",
        paid: "Paid",
      },
      mark_paid: "Mark paid",
      unmark: "Unmark",
      attach_txn: "Attach transaction",
      add_stage: "+ Stage",
      delete_stage: "Delete stage",
      delete_stage_confirm: "Delete this stage?",
      sum_mismatch_warning: "Stage totals do not match order amount",
      pay_account_label: "Account",
      pay_amount_label: "Payment amount",
    },
    work: {
      title_create: "New income source",
      title_edit: "Edit income source",
      delete: "Delete source",
      delete_confirm: "Delete this income source?",
      delete_failed: "Delete failed",
      currency_locked: "Currency is locked — transactions are linked",
      field: {
        kind: "Type",
        name: "Name / Employer",
        currency: "Currency",
        pay_day: "Pay day",
        tax_rate: "Tax rate (%)",
        hours_per_month: "Hours per month",
        is_active: "Active",
        notes: "Notes",
        rate_type: "Rate type",
        rate_amount: "Rate amount",
        rate_hourly: "Hourly rate",
        rate_monthly: "Net monthly salary",
        rate_per_task: "Per-task amount",
        rate_daily: "Daily rate",
        rate_commission: "Commission (%)",
        premium_amount: "Bonus (avg/mo)",
        premium_note: "Bonus note",
        started_at: "Started",
        ended_at: "Ended",
      },
      kind: {
        not_set: "— not set",
        employment: "Employment",
        freelance: "Freelance",
        one_time: "One-time",
      },
      section: {
        basics: "Basics",
        pay_details: "Pay details (reference)",
        period: "Period",
      },
      rate_type: {
        hourly: "Hourly",
        monthly: "Monthly",
        per_task: "Per task",
        daily: "Daily",
        commission_pct: "Commission %",
      },
      errors: {
        required_for_employment: "Required for employment",
        required_for_monthly: "Required for monthly rate",
        commission_out_of_range: "Commission must be 0–100%",
        ended_before_started: "End is before start",
        required_when_rate_set: "Pick a rate type",
      },
      placeholder: {
        name: "Employer or name",
        pay_day: "1–31",
        tax_rate: "6, 13...",
        hours_per_month: "160",
        notes: "Notes (optional)",
      },
      hint: {
        hours_per_month: "Default 160 h/mo",
        tax_rate: "Hint only — manage taxes manually",
        currency_default: "If not set, defaults to your main currency",
        pay_details_reference: "Reference only — actual income is calculated from linked transactions",
      },
    },
    business: {
      title_create: "New business",
      title_edit: "Edit business",
      delete: "Delete business",
      delete_confirm: "Delete this business?",
      delete_failed: "Failed to delete",
      currency_locked: "Currency can't be changed — linked transactions exist",
      field: {
        name: "Name",
        currency: "Currency",
        started_at: "Start date",
        is_active: "Active",
        note: "Note",
      },
      placeholder: {
        name: "Business name",
        note: "Note (optional)",
      },
      hint: {
        currency_default: "If not set, defaults to your main currency",
      },
    },
    tx_confirm: {
      title: "Confirm transaction",
      amount_label: "Amount",
      date_label: "Date",
      note_label: "Note",
      partial: "Partially",
      fully: "Fully",
      submit: "Confirm",
    },
    tx_row: {
      confirm: "Confirm",
      edit: "Edit",
      delete: "Delete",
      miss: "Miss",
      cancel: "Cancel",
      are_you_sure: "Are you sure?",
      yes_miss: "Yes, miss it",
      yes_cancel: "Yes, cancel it",
      delete_confirm_body: "Transaction will be deleted. This action cannot be undone.",
      yes_delete: "Yes, delete",
    },
    sub: {
      title_create: "New subscription",
      title_edit: "Edit subscription",
      field: {
        name: "Name",
        icon: "Icon",
        icon_color_fg: "Icon color",
        icon_color_bg: "Icon background",
        price: "Price",
        currency: "Currency",
        interval: "Interval",
        next_payment: "Next payment",
        sharing: "Sharing type",
        total_users: "Number of participants",
        family: "Family",
        note: "Note",
      },
      interval_options: {
        m1: "Monthly",
        m3: "Quarterly",
        m6: "Semi-annually",
        m12: "Annually",
      },
      sharing_types: {
        personal: "Personal",
        split: "Split (shared)",
        paid_for_others: "Paying for others",
      },
      field_is_variable: "Variable price",
      field_is_variable_help: "price changes each period",
      field_auto_match: "Auto-match",
      field_auto_match_help: "automatically link transactions",
      pay_button: "Pay",
      pay_confirm_title: "Confirm payment",
      pay_confirm_body: "Pay {name} — {amount}? Next payment will shift by {n} mo.",
      delete_confirm_title: "Delete subscription",
      delete_confirm_body: "Delete \"{name}\"? This action cannot be undone.",
      shares_editor: {
        title: "Sharing participants",
        add: "+ Add",
        field_person: "Name",
        field_amount: "Amount",
        delete: "Delete",
        empty: "No participants",
      },
      reimbursement: {
        section_title: "Reimbursement",
        field_expected: "Expected reimbursement",
        field_currency: "Reimbursement currency",
        field_from: "From whom",
        field_from_placeholder: "e.g. Victoria G.",
      },
    },
    loan: {
      title_create: "New loan",
      title_edit: "Edit loan",
      field: {
        name: "Name",
        principal: "Loan amount",
        annual_rate: "Annual rate (%)",
        term_months: "Term (months)",
        start_date: "Start date",
        currency: "Currency",
        account: "Payment account",
        note: "Note",
      },
      delete_confirm_title: "Delete loan",
      delete_confirm_body: "Delete \"{name}\"? This action cannot be undone.",
      payment_dialog: {
        title: "Record payment",
        field_paid_at: "Payment date",
        field_total: "Payment amount",
        field_principal: "Principal",
        field_interest: "Interest",
        field_account: "Debit account",
        field_note: "Note",
        auto_split: "Auto from schedule",
        auto_split_hint: "Fill from amortization schedule",
        submit: "Record payment",
      },
    },
    fund: {
      title_create: "New fund",
      title_edit: "Edit fund",
      field: {
        kind: "Fund type",
        name: "Name",
        currency: "Currency",
        goal_amount: "Goal amount",
        current_amount: "Already saved",
        target_date: "Target date",
        monthly_contribution: "Monthly contribution",
        note: "Note",
      },
      kind_options: {
        trip: "Trip",
        buy: "Purchase",
        vault: "Safety net",
        gift: "Gift",
        other: "Other",
      },
      delete_confirm_title: "Delete fund",
      delete_confirm_body: "Delete \"{name}\"? This action cannot be undone.",
      contribute_dialog: {
        title: "Add contribution",
        field_amount: "Amount",
        field_account: "Debit account",
        field_date: "Date",
        field_note: "Note",
        submit: "Contribute",
      },
    },
    event: {
      title_create: "New event",
      title_edit: "Edit event",
      field: {
        kind: "Type",
        title: "Name",
        event_date: "Date",
        repeats_yearly: "Repeats yearly",
        fund: "Linked fund",
        expected_amount: "Expected amount",
        currency: "Currency",
        note: "Note",
      },
      kind_options: {
        birthday: "Birthday",
        holiday: "Holiday",
        trip: "Trip",
        purchase: "Purchase",
        other: "Other",
      },
      delete_confirm_title: "Delete event",
      delete_confirm_body: "Delete \"{name}\"? This action cannot be undone.",
    },
    long_project: {
      title_create: "New project",
      title_edit: "Edit project",
      field: {
        name: "Name",
        budget: "Budget",
        currency: "Currency",
        category: "Category",
        start_date: "Start date",
        end_date: "End date",
        note: "Note",
      },
      placeholder: {
        name: "Project name",
        note: "Note (optional)",
        category: "— no category —",
        end_date: "no deadline",
      },
      delete_confirm_title: "Delete project",
      delete_confirm_body: "Delete \"{name}\"? This action cannot be undone.",
      has_transactions_hint: "Cannot delete project with transactions",
    },
    personal_debt: {
      title_create: "New debt",
      title_edit: "Edit debt",
      field: {
        direction: "Direction",
        counterparty: "Counterparty",
        principal: "Amount",
        currency: "Currency",
        opened_at: "Issue date",
        due_at: "Due date",
        account: "Account",
        note: "Note",
      },
      direction: {
        OUT: "I lent out",
        IN: "I borrowed",
      },
      placeholder: {
        counterparty: "Name",
        note: "Note (optional)",
        due_at: "no deadline",
        account: "— select account —",
      },
      delete_confirm_title: "Delete debt",
      delete_confirm_body: "Delete debt with \"{name}\"? This action cannot be undone.",
      has_transactions_hint: "Cannot delete debt with transactions",
      close_confirm_title: "Close debt",
      close_confirm_body: "Mark debt with \"{name}\" as closed?",
      payment_dialog: {
        title: "Record return",
        field_amount: "Amount",
        field_account: "Account",
        field_occurred_at: "Date",
        field_note: "Note",
        submit: "Record",
      },
    },
    family: {
      title_create: "Create family",
      title_edit: "Edit family",
      field: {
        name: "Name",
        note: "Note",
      },
      placeholder: {
        name: "Family name",
        note: "Note (optional)",
      },
      delete_confirm_title: "Delete family",
      delete_confirm_body: "Delete \"{name}\"? All group data will be deleted.",
    },
    family_member: {
      title_create: "Add member",
      title_edit: "Edit member",
      field: {
        display_name: "Name",
        letter: "Letter (avatar)",
        color: "Color",
        role: "Role",
      },
      role: {
        OWNER: "Owner",
        MEMBER: "Member",
      },
      placeholder: {
        display_name: "Member name",
        letter: "auto",
        color: "— select —",
      },
      remove_confirm_title: "Remove member",
      remove_confirm_body: "Remove \"{name}\" from the group?",
      owner_remove_hint: "Owner cannot be removed",
      guest_hint: "Member added as a guest — no system account",
    },
  },

  buttons: {
    add_subscription: "+ Subscription",
    add_loan: "+ Loan",
    add_fund: "+ Fund",
    add_event: "+ Event",
    add_project: "+ Project",
    add_debt: "+ Debt",
    edit: "Edit",
    delete: "Delete",
    pay: "Pay",
    contribute: "Contribute",
    add_payment: "+ Payment",
    shares: "Participants ({count})",
    add_member: "+ Add member",
    record_return: "Record return",
    close_debt: "Close",
    reopen_debt: "Reopen",
    confirm_delete: "Confirm delete",
    remove_member: "Remove member",
  },

  drawer: {
    close: "Close",
    placeholder_title: "Form will appear here",
    placeholder_body: "Implementation in progress",
  },

  signals: {
    section_title: "signals",
    subtitle: "hints, not actions",
    active_count: "{n} active",
    empty: "no active signals",
    empty_hint: "they'll appear on spending anomalies and upcoming charges",
    dismiss: "dismiss",
    item: {
      free_negative: {
        title: "no free funds",
        body: "spending from reserves — obligations exceed your free balance",
      },
      safe_until_critical: {
        title: "funds run out in {n} days",
        body: "at current burn rate the balance reaches zero — cut spending now",
      },
      month_deficit: {
        title: "monthly deficit: {amt}",
        body: "expenses exceed income — negative cash flow this month",
      },
      safe_until_low: {
        title: "runway: {n} days",
        body: "free funds will last less than three weeks",
      },
      obligation_due_soon: {
        title: "{label} in {n} days",
        body: "payment of {amt} — make sure funds are available",
      },
      obligation_load_high: {
        title: "obligations: {pct}% of free funds",
        body: "upcoming charges total {amt} — more than half of free balance",
      },
      category_overspend: {
        title: "{cat} +{pct}% vs last month",
        body: "spent {amt} — notable increase in this category",
      },
      expense_over_plan: {
        title: "expenses at {pct}% of plan",
        body: "actual {fact} already exceeds plan {plan}",
      },
      income_under_plan: {
        title: "income at {pct}% of plan",
        body: "actual {fact} is well below plan {plan}",
      },
      shrinkable: {
        title: "{cat}: overspend {pct}%",
        body: "{amt} above average — candidate for reduction",
      },
      discretionary_high: {
        title: "discretionary spending: {pct}%",
        body: "over half of expenses are flexible — room to optimise",
      },
      economy_exit_slow: {
        title: "economy exit: {n} months",
        body: "in Economy mode it will take several months to restore your balance",
      },
      mode_free_risky: {
        title: "free mode with unstable finances",
        body: "current status is not stable — consider switching to Normal or Economy",
      },
      no_outflow_plan: {
        title: "no expense plan set",
        body: "actual {amt} is tracked but there is nothing to compare against — add a budget",
      },
      no_inflow_plan: {
        title: "no income plan set",
        body: "without a planned income it is hard to assess deviations",
      },
      category_new: {
        title: "new category: {cat}",
        body: "first spend of {amt} in this category this month",
      },
      multi_currency: {
        title: "multi-currency wallet: {n} currencies",
        body: "watch exchange rates — changes affect the total balance in base currency",
      },
      burn_rate: {
        title: "daily spend: {amt}",
        body: "average daily burn derived from free funds and remaining runway",
      },
      safe_until_good: {
        title: "financial runway is healthy",
        body: "free funds will last more than 90 days at current pace",
      },
      month_surplus: {
        title: "monthly surplus: {amt}",
        body: "income exceeds expenses — consider directing the remainder to goals",
      },
      obligations_clear: {
        title: "no charges in the next 30 days",
        body: "no mandatory payments due within the month horizon",
      },
    },
  },

  home: {
    top_categories: {
      title:       "top categories",
      delta_label: "vs prev month",
      meta:        "top {count} · desc",
      empty:       "no category spending data",
      // === wave-3 polish ===
      delta_new: "new",
    },
    plan_fact: {
      title: "plan-fact",
      meta:  "current month",
      code_inc: "INCOME",
      code_exp: "EXPENSE",
      code_net: "NET",
      // === wave-3 polish ===
      no_plan: "no plan set",
      no_plan_set: "no plan set",
      actual_without_plan: "actual without plan",
      of_amount: "of {amount}",
      net_eom_sub: "end of month ≈ {amount}",
      period_label: {
        "7d":  "last 7d",
        "30d": "last 30d",
        "90d": "last 90d",
        "1y":  "last year",
      },
      period_meta: {
        "7d":  "last 7 days",
        "30d": "current month",
        "90d": "last 90 days",
        "1y":  "last year",
      },
    },
    status_strip: {
      mode_label:   "MODE",
      period_label: "PERIOD",
      modes: {
        econom: "Econom",
        normal: "Normal",
        free:   "Free",
      },
      day_hint:   "day of month / days in month",
    },
    quick: {
      title: "quick actions",
      hotkey_hint: "hotkeys active · Q",
      income: {
        label: "Income",
        aria: "add income",
      },
      expense: {
        label: "Expense",
        aria: "add expense",
      },
      transfer: {
        label: "Transfer",
        aria: "create transfer",
      },
      transaction: {
        label: "Transaction",
        aria: "add transaction",
      },
      shortcut_hint: "I / E / T for quick actions",
    },
    obligations: {
      title: "obligations",
      period_30d: "next 30d",
      count_suffix: "items",
      empty: {
        text: "no obligations in the next 30 days",
        cta: "+ obligation",
      },
    },
  },

  quick_input: {
    title: "quick entry",
    placeholder: "1500 coffee yesterday / salary 80000 today",
    confirm: "Record",
    edit: "Edit",
    open_full: "Open full form",
    hotkey_hint: "Q",
    preview: {
      kind: "kind",
      amount: "amount",
      category: "category",
      date: "date",
      account: "account",
    },
    kind: {
      income: "income",
      expense: "expense",
    },
    error: {
      no_amount: "Could not parse amount",
      no_account: "Select an account",
      invalid: "Invalid input",
    },
    warning: {
      no_category: "Category not recognized",
      multiple_amounts: "Multiple amounts found — using first",
      currency_mismatch: "Currency differs from account",
      category_kind_mismatch: "Category does not match transaction type",
    },
    no_accounts: "No active accounts. Create an account in Wallet.",
  },

  dialog: {
    confirm_yes: "Yes",
    confirm_no: "Cancel",
  },

  onboarding: {
    title: "welcome",
    subtitle: "let's set up your budget in a few steps",
    name: {
      label: "What is your name?",
      placeholder: "Your name",
    },
    gender: {
      label: "Gender",
      male: "Male",
      female: "Female",
      unspecified: "Prefer not to say",
    },
    currency: {
      label: "Primary currency",
      hint: "Used for calculations and balance display",
    },
    mode: {
      label: "Budget mode",
      economy: "Economy",
      normal: "Normal",
      free: "Free",
      hint_economy: "Strict limits, savings mode",
      hint_normal: "Standard limits for most people",
      hint_free: "Soft limits, no restrictions",
    },
    submit: "Get started",
    error_save: "Save error. Please try again.",
  },

  shell: {
    brand: "БДЖ://",
    status: {
      stable:  "STABLE",
      warning: "WARNING",
      crisis:  "CRISIS",
    },
    crumbs: {
      home: "overview",
      txn:  "feed",
      inc:  "sources",
      exp:  "obligations",
      plan: "calendar",
      anl:  "weather",
      wal:  "accounts",
      fam:  "group",
      biz:  "P&L",
      // === wave-2 parser+wallet+breadcrumbs ===
      settings: "profile & budget",
      settings_sub: {
        categories:   "categories",
      },
      wal_sub: {
        integrations: "integrations",
      },
    },
    nav: {
      tabs: {
        home:     "Home",
        txn:      "Transactions",
        inc:      "Income",
        exp:      "Expenses",
        plan:     "Planning",
        anl:      "Analytics",
        wal:      "Wallet",
        fam:      "Family",
        biz:      "Business",
        settings: "Settings",
      },
      codes: {
        home:     "HOM",
        txn:      "TXN",
        inc:      "INC",
        exp:      "EXP",
        plan:     "PLN",
        anl:      "ANL",
        wal:      "WAL",
        fam:      "FAM",
        biz:      "BIZ",
        settings: "SET",
      },
      aria: "Primary navigation",
    },
    resize: {
      aria_rail:    "Navigation width",
      aria_summary: "Summary width",
      title:        "Drag to resize · double-click to reset",
    },
    clock: {
      updated: "upd",
      // === wave-3 polish ===
      tooltip: "Local time",
    },
    summary: {
      aria: "Summary",
      safe: {
        label:        "safe for",
        label_unsafe: "RUNWAY OUT",
        zero_income:  "@ zero income",
        days:         "days",
      },
      avail: {
        label:    "available now",
        total:    "total",
        reserved: "reserved",
      },
      balances: {
        label:    "balances",
        empty:    "no data",
        accounts: "{count} accounts",
      },
      session: {
        label:  "session state",
        online: "online",
        status: "status",
        integrations: "integrations",
      },
      quick: {
        label:       "quick",
        income:      "+ Income",
        expense:     "+ Expense",
        txn:         "+ Tx",
        tip_income:  "Income (I)",
        tip_expense: "Expense (E)",
        tip_txn:     "Transaction (T)",
      },
      cashflow: {
        title:          "cash flow 30d",
        period_label:   "current period",
        empty:          "no data",
      },
    },
    sync: {
      busy:    "Sync already in progress",
      error:   "Sync error",
      empty:   "No integrations",
      tooltip: "Sync",
    },
  },

  import: {
    title: "CSV import",
    step_upload: "upload",
    step_preview: "preview",
    step_result: "result",
    source: {
      label: "Source",
      tinkoff: "Tinkoff",
      generic: "Other bank (CSV)",
    },
    encoding: {
      label: "File encoding",
      utf8: "UTF-8",
      win1251: "Windows-1251",
    },
    delimiter: {
      label: "Delimiter",
      semicolon: "Semicolon (;)",
      comma: "Comma (,)",
    },
    file: {
      label: "Upload .csv file",
    },
    csv_paste: {
      label: "Or paste CSV text",
      placeholder: "Paste the CSV file contents here...",
    },
    mapping: {
      title: "Column mapping",
      date: "Date",
      amount: "Amount",
      currency: "Currency",
      category: "Category",
      description: "Description",
    },
    files: {
      add: "Add files",
      drop_hint: "or drop CSV files here",
      remove: "Remove file",
      target_account: "Target account",
      source: "Format",
      row_count: "{n} rows",
    },
    cardhint: {
      title: "Unknown cards",
      body: "Card *{last4} is not linked. Link to {account}?",
      bind: "Link",
      bound: "Linked",
    },
    preview: {
      included: "Include",
      duplicate: "Duplicates",
      total: "Total rows",
      will_import: "Will import",
      duplicate_hint: "Likely already exists in database",
      transfer_no_category: "—",
      truncated_hint: "Showing 500 of {n} rows. All rows will be processed on import.",
      intra_skipped_group: "Intra-account transfers (skipped): {n}",
      paired_badge: "↔ paired",
      unpaired_badge: "⚠ unpaired",
      unpaired_hint: "Self-transfer with no matching counterpart — will be imported as a regular income/expense",
      source_file: "Source",
    },
    confirm: {
      button: "Import {n}",
      button_with_transfers: "Import {n} (transfers: {t})",
    },
    back: "Back",
    next: "Next",
    result: {
      title: "Import complete",
      created: "Created",
      transfers_created: "Transfers created: {n}",
      skipped: "Skipped",
      errors: "Errors",
      go_transactions: "Go to transactions",
      error_row: "row {index}: {message}",
    },
    warning: {
      more: "more warnings",
      bad_row: "invalid row",
      unknown_currency: "unknown currency",
      mapping_required: "mapping not set",
      parse_error_row: "Parse error in row {row}: {message}",
      bad_row_detail: "Row {row}: invalid data ({reason})",
      skipped: "Skipped row {row}: {reason}",
      with_file: "{file}: {body}",
    },
    error: {
      no_csv: "Please paste or upload a CSV file",
      no_account: "Please select an account",
      mapping_incomplete: "Please specify date and amount columns",
    },
  },
  tooltip: {
    abbrev: {
      obn: "updated",
    },
  },
  debts: {
    dir: {
      out: "lent",
      in: "borrowed",
    },
    next_payment: "next {amount} on {date}",
    next_payment_no_date: "next {amount} on —",
    returns_count: "{n} return{suffix}",
    returns_suffix: {
      one: "",
      few: "s",
      many: "s",
    },
    no_returns: "no returns",
    progress_label: "returned {returned} / {total}",
  },
};

/**
 * Plural forms for English nouns used with pluralEn() helper.
 * Stored outside enDict because the i18n Dict type only allows string leaves.
 * Format: [singular, plural]
 */
export const enPluralForms = {
  locations:     ["location",     "locations"]     as [string, string],
  currencies:    ["currency",     "currencies"]    as [string, string],
  accounts:      ["account",      "accounts"]      as [string, string],
  organizations: ["organization", "organizations"] as [string, string],
  transactions:  ["transaction",  "transactions"]  as [string, string],
  funds:         ["fund",         "funds"]         as [string, string],
  events:        ["event",        "events"]        as [string, string],
  subscriptions: ["subscription", "subscriptions"] as [string, string],
  categories:    ["category",     "categories"]    as [string, string],
  institutions:  ["institution",  "institutions"]  as [string, string],
  sources:       ["source",       "sources"]       as [string, string],
  projects:      ["project",      "projects"]      as [string, string],
} as const;
