/**
 * Budget Tracker — demo seed (Никитины, Тинькофф/Сбер/Альфа, ипотека, 8 подписок и т.п.).
 *
 * Заливает полный демо-датасет для единственного пользователя (DEFAULT_USER_ID).
 * Каждый прогон очищает user и создаёт заново — безопасно гонять многократно в dev.
 * Справочники (Currency, ExchangeRate) пере-заливаются тоже.
 *
 * Запуск: `npm run seed:demo`.
 *
 * NB: reference-seed (Currency + ExchangeRate, без user) живёт отдельно в prisma/seed.ts
 * и запускается через `npm run seed` и автоматически при `prisma migrate reset`.
 */
...
async function seedUser() {
  await db.user.create({
    data: {
      ...
      gender: Gender.MALE,
      // demo-flow считается уже онбордженным — wizard не показывается.
...
// ────────────────────────────────────────────────────────────────
// 4. Categories (базовый набор + лимиты для 3 режимов)
// ────────────────────────────────────────────────────────────────
const CATEGORY_IDS = {
  salary: "cat_salary",
...
async function seedCategories() {
  ...
  // Expenses. Лимиты для 3 режимов (Эконом / Нормальный / Свободный) заданы где релевантно.
  await db.category.createMany({
    data: [
